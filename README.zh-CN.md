...
Server 启动后会在后台导入数据，页面不需要等待扫描完成才能打开。

| 来源 | 默认本机位置 | 导入时机 |
| --- | --- | --- |
| Claude Code | `~/.claude/projects` | 启动时与“重新扫描” |
| Codex | `~/.codex/sessions` | 启动时与“重新扫描” |
| Zed | 本机 Zed `threads.db` | 数据库存在时启动扫描 |
| MiMo | 本机 `mimocode.db` | 数据库存在时启动扫描 |

如果列表为空，点击首页的**重新扫描**。它会扫描默认的 Claude Code 与 Codex 目录，并
显示新增、更新、跳过和失败数量。Zed 与 MiMo 会在其数据库存在时于启动阶段自动发现。
...
2. 等待启动扫描完成，再点击**重新扫描**导入 Claude Code 与 Codex。
