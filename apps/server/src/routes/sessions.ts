...
function scoreSession(
  ...
  return calcEfficiencyScore(
    efficiency,
    session.cacheHitRate,
    totalTokens,
    session.outputTokens,
    session.totalCost,
    result.totalWastedCost,
  );
}
...
export function registerSessionRoutes(app: FastifyInstance, runtime: SessionRuntime): void {
  ...
  app.patch<{ Params: { id: string }; Body: { tags?: string; notes?: string } }>(
    '/api/session/:id',
    async (req, reply) => {
      const session = db.prepare('SELECT id FROM sessions WHERE id = ?').get(req.params.id) as
        | { id: string }
        | undefined;
      if (!session) return reply.status(404).send({ error: 'session not found' });
      const { tags, notes } = req.body;
      if (tags !== undefined)
        db.prepare('UPDATE sessions SET tags = ? WHERE id = ?').run(tags, req.params.id);
      if (notes !== undefined)
  ...
  app.get<{ Params: { id: string } }>('/api/session/:id/cost-attribution', async (req, reply) => {
    ...
    const spans = rows.map(parseSpanRow);
    const detail = { ...session, spans } as SessionDetail;
    const diag = diagnoseSessionSync(detail, {
    ...
    async (req, reply) => {
      ...
      if (format === 'csv') {
        ...
        return reply
          ...
          .header(
            'Content-Disposition',
            `attachment; filename="session-${session.id.slice(0, 8)}.csv"`,
          )
          .send([headers, ...lines].join('\n'));
      }
  ...
  app.get<{ Params: { id: string } }>('/api/session/:id/commits', async (req, reply) => {
    const session = db
      .prepare(
        `SELECT id, start_time as startTime, end_time as endTime, cwd FROM sessions WHERE id = ?`,
      )
      .get(req.params.id) as
      | { id: string; startTime: number; endTime?: number; cwd?: string }
      | undefined;
    if (!session) return reply.status(404).send({ error: 'session not found' });
  ...
  app.get<{ Params: { id: string } }>('/api/session/:id/report', async (req, reply) => {
    ...
    const lines = [
      ...
      '## Top Tools',
      '',
      ...eff.toolSuccessRates
        .slice(0, 10)
        .map(
          (t) => `- **${t.name}**: ${t.total} calls, ${(t.successRate * 100).toFixed(0)}% success`,
        ),
      '',
      '## Diagnosis',
      '',
  ...
  app.get<{ Querystring: { ids: string } }>('/api/sessions/compare', async (req, reply) => {
    ...
    const sessions = ids
      .map((id) => {
        const s = db.prepare(`SELECT ${SESSION_COLS} FROM sessions WHERE id = ?`).get(id) as
          | SessionSummary
          | undefined;
        if (!s) return null;
        const rows = db
