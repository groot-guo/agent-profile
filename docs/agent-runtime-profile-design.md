...
| Configuration | 本次运行采用的模型、规则、工具策略和提示模板版本 | 缺失 |
...
第一阶段仅采集结构化质量特征：是否包含目标、范围、验收、限制、相关上下文、验证要求。只有用户主动授权，才保存经过脱敏的 prompt 文本。

提示词或 Agent 规则建议的前提：同类 cohort 有足够样本，并同时满足质量 guardrail。例如推荐“要求运行测试”之前，需证明测试通过率/返工率改善且成本没有超过上限。
...
人类可读差异页；诊断卡片与完整事件时间线的统一透明度仍属于后续任务。该报告只基于
Session/Span，Outcome 明确标记为未采集。
