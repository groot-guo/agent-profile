...
`sessions` table add `agent` column (`claude-code` | `codex` | `zed`). Requires deleting `server/trace.db` (schema rebuild, `CREATE TABLE IF NOT EXISTS` won't alter existing).
