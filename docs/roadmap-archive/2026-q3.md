...
### T159 GitHub CI smoke build alignment

- status: completed
- started_at: 2026-08-12
- completed_at: 2026-08-12
- implemented scope: hardened `.github/workflows/ci.yml` with read-only
  repository permissions and explicit Node/pnpm version diagnostics; changed
  the Playwright smoke job to run `pnpm build:release` and start the resulting
  CLI/Web standalone build without a second Next build; documented the CI
  contract in README and ARCHITECTURE.
- decisions: CI remains content-free and disposable. The smoke job now checks
  the same standalone packaging path as local distribution checks, while no
  release archive is uploaded or published.
- acceptance result: verify checks remain unchanged; the smoke job has one
  deterministic standalone build, installs Chromium, and runs the four bounded
  smoke tests against loopback/disposable data.
- changed files: `.github/workflows/ci.yml`, `apps/web/playwright.config.ts`,
  `README.md`, `ARCHITECTURE.md`, `docs/roadmap.md`, and this archive
- verification:
  - `pnpm build:release` — passed; standalone Web, CLI bundle, and disposable
    current-platform release directory were generated
  - `pnpm --filter agent-profile-web exec playwright test` — 4 passed
  - `pnpm test` — Core 284, Web 63, Server 190, CLI 33 passed
  - `pnpm lint`, `pnpm check:roadmap`, `pnpm check:boundaries`, and
    `pnpm benchmark:scale:ci` — passed
  - `git diff --check` — passed
- known limits: hosted GitHub runner execution was not directly queried because
  this environment has no GitHub CLI authentication/network access; the local
  commands reproduce both CI build and smoke paths.
...
| P1 quantified diagnosis (11 deterministic rules + API + web) | done                          |
...
| data source                                                   | Claude Code + Codex + Zed + MiMo + OpenCode |
...
### T108 Model Catalog bundled-seed startup recovery

- status: completed
- started_at: 2026-07-31
- completed_at: 2026-07-31
- estimated size/risk: small / high; startup was blocked on an existing local
  database and repair had to retain user pricing/history without deletion
- purpose: recover Server startup when a failed early T99 seed left bundled
  `NULL effective_from` rows beside existing epoch-zero applicability rows
- scope:
  - reproduce repeated real-database residue in a regression test
  - supersede only conflicting bundled residue and normalize non-conflicting
    bundled rows
  - add an ordered recovery migration for locally recorded early-v8 Span
    provenance
  - exclude superseded rows from current configuration export/listing
  - verify startup and repeat-start idempotency against a copy of the real
    database
- acceptance:
  - Server starts without deleting pricing or history
  - exact existing epoch-zero user/legacy rows remain selected
  - no duplicate current schedule or superseded residue is exposed
  - normal fresh seeding and T99 history/provenance behavior remain unchanged
- completion evidence:
  - added migration v9, `model_catalog_span_provenance_recovery`, to restore
    missing `spans.pricing_model` and `spans.pricing_revision` columns when an
    earlier local database had already recorded the evolving v8 migration
  - startup now retains repeated bundled `NULL` residue as `superseded`, keeps
    existing epoch-zero rows authoritative, and omits superseded rows from
    current pricing/configuration responses
  - a two-start smoke test against a `/private/tmp` copy of the real database
    returned health 200 both times, retained 319 pricing rows and 29 history
    rows, migrated to v9 with both Span columns, retained all 290 residues as
    superseded, and exposed only the 29 current schedules
  - updated `ARCHITECTURE.md` with the durable bundled-seed recovery behavior
  - `pnpm lint`, `pnpm build`, and `pnpm test` passed; the full test result was
    30 Core files/208 tests, 26 Server files/107 tests, 6 Web files/23 tests,
    and 4 CLI files/28 tests
  - `pnpm check:boundaries`, `pnpm check:roadmap`, and `git diff --check` passed
- known limits:
  - historical residue remains stored for provenance and is intentionally not
    deleted; it is excluded from current schedule and configuration surfaces
...
- implemented scope:
  - added `/settings/models` and the global `模型` navigation entry, consuming
    only the public `model-catalog/v1` API
  - prioritized observed unsupported/unpriced models, retained exact raw-model
    URL identity, and exposed observation coverage, current source, history,
    context provenance, alias evidence, and user-override state
  - added validated four-token-class price and exact context-window editors with
    effective local-to-UTC disclosure and source/audit references
  - kept configuration saves separate from historical mutation; scoped preview
    shows Span/Session and known/unknown coverage, execute requires an explicit
    checkbox, reports pending/result state, and clears stale-revision previews
  - added versioned local JSON configuration import/export without Session or
    prompt content
- completion evidence:
  - implemented the feature in `apps/web/app/settings/models/page.tsx`, its CSS
    module, and the tested pure view model in `model-catalog.ts`; updated
    `apps/web/app/header.tsx`
  - added RED/GREEN coverage for priority sorting, price validation and local
    effective-time conversion, and stale preview revision gating
  - browser verification against the real local inventory confirmed safe empty
    price validation, read-only preview with execute disabled by default, model
    URL deep links, desktop and 375px layouts, light/dark themes, no horizontal
    overflow, and no browser warnings/errors; no pricing/context write or execute
    was performed during verification
  - updated `README.md`, `README.zh-CN.md`, `ARCHITECTURE.md`,
    `docs/zh/OVERVIEW.md`, `docs/ui-guidelines.md`, and
    `docs/model-configuration-design.md` for implemented behavior
  - `pnpm lint`, `pnpm build`, and `pnpm test` passed; tests covered 30 Core
    files/208 tests, 26 Server files/107 tests, 7 Web files/27 tests, and 4 CLI
    files/28 tests
  - `pnpm check:boundaries`, `pnpm check:roadmap`, and `git diff --check` passed
- known limits:
  - the workspace displays existing explicit pricing aliases and preserves them
    through configuration import/export; direct alias editing remains available
    through the public API rather than a dedicated Web editor
  - the Web reports request-level execute progress because the current Server
    contract is synchronous; it does not invent record-level percentages
...
- documentation plan: update `ARCHITECTURE.md`, README/configuration guidance,
  and this Task
- implementation:
  - confirmed the existing T58 vendor-catalog audit and retained its corrected
    20-row seed values, including `qwen-max=131072`; expanded the seed comments
    with the remaining provider entry points without overwriting user rows
  - documented the owner, scope, provenance, and unknown behavior of
    `model_context`, `pricing`, deterministic diagnosis thresholds, and Task
    Configuration Snapshots in the current architecture and bilingual guides
  - made the diagnosis price decision explicit: stored cost remains
    span-time-priced, while diagnostic `wastedCost` remains a current
    analysis-time input-price upper-bound estimate for planning
  - added a Server regression check proving an unknown raw model does not
    inherit a guessed context window
- verification:
  - Server tests passed (24 files / 92 tests), including application-factory,
    pricing-route, seed, and unknown-model coverage
  - Core tests passed (30 files / 206 tests), and Core/Server builds passed
  - `pnpm exec biome check .`, `pnpm check:roadmap`, and `git diff --check`
    passed
- known limitation:
  - model-context provenance is currently documented as audited vendor entry
    points rather than persisted per-row source/version metadata; T99 remains
    responsible for the versioned Model Catalog provenance contract
...
- shipped scope:
  1. added additive migration v8 for pricing/context provenance and revision,
     pricing history, explicit pricing aliases, selected-price Span provenance,
     and recalculation audit
  2. moved bundled reference data into `apps/server/src/model-catalog/defaults.ts`;
     bundled, manual, imported, and migrated legacy rows remain distinct and
     bundled seeding does not overwrite a local applicability key
  3. retained exact raw-model pricing first and added only explicit audited
     `pricingEquivalent=true` fallback; provider and presentation aliases are
     not price aliases
  4. added observed-model inventory and `model-catalog/v1` recalculation
     preview/execute with normalized model/time scope, before/after unknown
     coverage, a fixed pricing revision, transactional Span and Session updates,
     selected price provenance, rollback, and completed-run audit
  5. adapted `/api/pricing`, `/api/model-context`, and `/api/recompute-cost` to
     the same service while retaining their request/response behavior
  6. added versioned local configuration import/export for prices, contexts,
     and aliases without Session or prompt content; the full payload is
     validated before mutation
- acceptance result:
  - the four token classes and Span-time effective lookup remain separate and
    correct; unsupported schemes stay unknown in preview and execution
  - migrated rows, generated costs, compatibility endpoints, and generated-data
    reset behavior survive without source re-import or database deletion
  - selected prices now persist model key, revision, applicability time,
    currency, calculation time, and calculator version alongside source metadata
  - preview is read-only; execute rejects stale revisions, uses the same scope
    transactionally, rebuilds affected Session totals, and records the run
- changed components:
  - Model Catalog service/defaults/types and routes under `apps/server/src/`
  - migration/schema, Runtime composition, compatibility routes, Session
    repository, and selected-price Core fields
  - `model-catalog/v1` contracts in `packages/contracts/src/model-catalog.ts`
  - migration, service, route, compatibility, import/export, reset, stale
    revision, unsupported-scheme, and rollback tests
  - `ARCHITECTURE.md`, `README.md`, `docs/stats.md`,
    `docs/model-configuration-design.md`, and `docs/zh/OVERVIEW.md`
- verification result:
  - `pnpm lint` passed with zero diagnostics
  - `pnpm build` passed for Contracts, Core, Server, Web production, and CLI
  - `pnpm test` passed: Core 208, Server 104, Web 23, and CLI 28 tests
  - `pnpm check:boundaries`, `pnpm check:roadmap`, and `git diff --check` passed
  - local in-memory service/HTTP smoke is covered by the Model Catalog route
    tests, including configuration round-trip and recalculation preview/execute
- known limits:
  - the first calculator scheme remains flat four-token-class CNY pricing;
    unsupported tiered/batch/regional schemes are stored only as unsupported and
    cannot produce trusted cost
  - T100 owns the Web configuration workspace; T99 ships the Server contract
    and compatibility APIs only
...
- dependencies/assumptions: T104 URL/history semantics and T105's bounded
  analysis-summary endpoint were retained. Request-path inspection confirmed
  the normal flow is one bounded discovery request followed by one bounded
  analysis request; no sequential or duplicate request was found to remove.
- implemented scope:
  - added a small validated same-origin readiness protocol between the Home
    iframe host and embedded Session detail. The host accepts messages only from
    its current iframe, current origin, and selected Session ID.
  - retained the selected Session title and reserved content layout while the
    detail reads analysis; replacing the iframe on selection prevents a prior
    document from being displayed as the current detail.
  - exposed a bounded loading status without a timer or page animation, and a
    visible recovery message when the active detail reports failure.
- acceptance result:
  - selection immediately changes the visible Home content while preserving
    identity and return navigation; a child document's initial `load` does not
    incorrectly indicate that analysis data is ready.
  - malformed, foreign, and stale-ID readiness messages are rejected; direct
    detail URLs, Server routes, Session metrics, and live-refresh requests are
    unchanged.
- documentation: updated `ARCHITECTURE.md` and `docs/ui-guidelines.md` with
  the readiness, failure, and stale-message contract.
- verification:
  - RED/GREEN transition-protocol test passed, including malformed and stale
    message rejection; complete Web suite passed (8 files / 29 tests).
  - `pnpm lint`, `pnpm --filter agent-profile-web build`,
    `pnpm check:roadmap`, and `git diff --check` passed.
  - browser interaction verification was attempted against `127.0.0.1:3001`,
    but the Browser runtime could not connect to the local listener and then
    timed out in a newly created tab. This is unverified in that environment;
    no data-import, configuration, or reset action was performed.
- changed files: `apps/web/app/page.tsx`,
  `apps/web/app/session/[id]/page.tsx`, `apps/web/app/session-detail-transition.ts`,
  `apps/web/app/session-detail-transition.test.ts`, `apps/web/app/layout.tsx`,
  `ARCHITECTURE.md`, `docs/ui-guidelines.md`, and the roadmap register/archive.
...
- started_at: 2026-08-02
- completed_at: 2026-08-02
- purpose: make every Outcome field required for verified delivery evidence
  usable from the local Task workspace without changing the existing local-only
  persistence boundary.
- implemented scope:
  - extended the Tasks Outcome form with human rating, rework reason, local
    completion time, and add/remove bounded structured evidence rows alongside
    build/test/lint/Git fields.
  - added a pure browser payload converter that preserves blank values as null,
    converts local completion time to its exact timestamp, and rejects a
    partially entered evidence row instead of silently dropping it.
  - validated optional evidence status and finite completion time at the
    repository boundary, so direct API callers receive the same rejection.
  - retained the canonical `task-profile/v1` coverage display; it is not
    recomputed in the browser and still distinguishes missing from failed.
- acceptance result:
  - users can now create/update every supported Outcome field from the
    workspace; evidence items hold only a kind, optional verification status,
    and bounded reference; the form does not collect Prompt or rule-body fields.
  - Core smoke coverage proves not-collected, partial with explicit failed, and
    verified Outcome states; invalid structured evidence and completion times
    are rejected by both the form payload and repository contracts.
- documentation: updated README (English/Chinese), architecture, Task/Outcome
  guide, Chinese overview, and the roadmap register/archive.
- verification:
  - `pnpm --filter agent-profile-web test -- task-outcome.test.ts` passed
    (9 files / 31 tests).
  - `pnpm --filter trace-server test -- tasks.test.ts` passed (26 files / 108
    tests), and Core Task Profile tests cover missing, partial/failed, and
    verified coverage.
  - `pnpm --filter agent-profile-web build`, full lint/test, roadmap validation,
    boundary check, and `git diff --check` passed.
- changed files: Tasks page and Outcome helper/tests, Task repository/route
  tests, Core Task Profile smoke tests, README documentation, architecture,
  Task/Outcome guide, Chinese overview, and roadmap register/archive.
...
- completed_at: 2026-08-02
- implemented scope:
  - added Task-workspace loading, creation, viewing, and editing for local
    Cohort scopes and Experiment definitions using only existing public APIs.
  - Cohorts make project, Task-type, and complexity scope visible. Experiments
    select existing Cohort/control/candidate records, primary metric, and
    newline-separated guardrails.
  - an Experiment's Cohort and control/candidate configuration remain fixed
    during edits, matching the existing update API contract.
  - decision controls expose evidence state and retain the existing backend
    ready-evidence guardrail: `keep` and `rollback` are unavailable until
    `ready`, and the persistence constraint remains authoritative.
- acceptance result:
  - the UI describes definitions only and never calculates a distribution,
    winner, or causal claim. Raw Prompt/rule bodies are not collected by the
    form; scopes and missing Outcome limitations remain part of Task Profile.
- documentation: updated README (English/Chinese), architecture, Profile model,
  Task/Outcome guide, and roadmap register/archive to distinguish persisted
  definitions from future statistical evaluation.
- verification:
  - focused Server guardrail tests passed (26 files / 108 tests); complete Web
    tests passed (10 files / 32 tests), including the UI decision-availability
    guardrail; the production Web build passed.
  - full lint/test, boundary and roadmap checks, and `git diff --check` passed.
- changed files: `apps/web/app/tasks/page.tsx`,
  `apps/web/app/experiment-guardrail.ts`, its focused test, current
  Task/Cohort documents, and roadmap register/archive.
...
- purpose: reduce repeated JSONL parsing for growing Claude Code and Codex
  histories without weakening revision, privacy, annotation, or atomic
  replacement guarantees
- implemented scope:
  - added a bounded process-local checkpoint in the transcript adapter, retaining
    only a prefix digest, complete-line boundary, normalized Session identity,
    final-turn structural IDs, and source metadata needed to parse a suffix
  - added strict suffix validation for unchanged prefix bytes, monotonic
    timestamps, valid JSON lines, and source-specific independent-turn boundaries
  - Claude suffixes with cross-boundary tool results and Codex suffixes that
    continue a previous turn fall back to full parsing; rewrites, truncations,
    cache misses, malformed suffixes, and forced rebuilds use the existing full
    parse path
  - added transactional Session append support that analyzes new Spans, closes
    the previous final turn, refreshes token/cost/context/message aggregates,
    rejects stale revisions and duplicate Span IDs, and preserves annotations
  - extended parser options for suffix Session identity/metadata without
    persisting raw transcript or prompt content in the checkpoint
- acceptance result:
  - valid Claude and Codex appends produce the same normalized Span boundaries,
    token aggregates, and Session message counts as the equivalent full parse
  - prefix rewrites, malformed suffixes, incomplete Codex turns, and stale or
    conflicting append candidates safely use full replacement; parser/I/O
    failures retain the previous generated Session through the existing
    coordinator behavior
  - Zed, MiMo, and OpenCode adapters remain unchanged and no schema migration was
    added
- changed files:
  - `packages/core/src/scanners/transcript.ts`, parser options, and Core exports
  - `apps/server/src/ingestion/transcript-adapter.ts`, import coordinator/types,
    and Session repository append transaction
  - focused ingestion tests covering append, rewrite, malformed suffix, and
    Codex boundary fallback
  - `ARCHITECTURE.md`, `docs/multi-agent.md`, both READMEs, and roadmap archive
- verification:
  - Core 32 files / 214 tests passed; Server 27 files / 116 tests passed
  - `pnpm lint`, `pnpm build`, `pnpm check:boundaries`, `pnpm check:roadmap`,
    and `git diff --check` passed
  - ingestion fixtures verified append-vs-full Span results, prior-turn end
    time closure, aggregate refresh, rewrite fallback, malformed-line fallback,
    and incomplete Codex-turn fallback
- known limits:
  - checkpoints are process-local and bounded; process restart or eviction
    performs a normal full parse
  - only suffixes proven independent are incrementally appended; cross-boundary
    tool-result and partial-turn cases intentionally trade throughput for the
    existing full-parser correctness path
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: provide a coverage-aware Project Profile for observed resource,
  source, tool-reliability, and trend evidence without presenting process
  metrics as repository quality, delivery success, or configuration causality
- implemented scope:
  - added the versioned Core `project-profile/v1` contract and builder for one
    normalized `project_key` across primary Sessions, including resource totals,
    trusted cost coverage, cache/context/duration coverage, Agent/source scope,
    tool frequencies/errors, UTC day trends, and explicit limitations
  - added bounded read-only `GET /api/projects/profile?project=...&from=...&to=...`
    over existing Session/Span storage; the route accepts an optional exclusive
    millisecond time range, keeps at most 1,000 newest matching Sessions and
    10,000 tool events, and marks sampled scopes
  - added a Project Profile section to the existing Stats workspace and a
    navigable `/projects` page with project selection, loading/error/empty states,
    coverage labels, resource summary, tool reliability, and recent day trends
  - preserved missing source, pricing, metric, tool, and file evidence as
    coverage limitations; file-level evidence remains `not_captured` and no raw
    prompt or transcript content is persisted or returned
- acceptance result:
  - every report exposes project key, requested range, linked/available Session
    counts, sampling state, Agent/source coverage, metric coverage, tool error
    evidence, trends, and limitations
  - primary Session and source-native relationship boundaries remain intact;
    no schema migration, repository-wide file claim, quality verdict, or causal
    winner was introduced
- changed files:
  - `packages/core/src/project-profile.ts`, its index export, and focused Core test
  - `apps/server/src/routes/project-profile.ts`, route registration, and focused
    Server API/coverage tests
  - `apps/web/app/stats/page.tsx`
  - `ARCHITECTURE.md`, `docs/profile-model.md`, `docs/stats.md`, `README.md`,
    `README.zh-CN.md`, `docs/zh/OVERVIEW.md`, and the roadmap register/archive
- verification:
  - Core 31 files / 212 tests passed; Server 27 files / 112 tests passed;
    Web 12 files / 36 tests passed
  - `pnpm lint`, `pnpm check:boundaries`, `pnpm check:roadmap`, `pnpm build`,
    and `git diff --check` passed
  - focused route tests covered project/time-range filtering, source and tool
    coverage, privacy-safe response shape, and invalid/inverted ranges; Web
    production build verified the Stats Project Profile surface
- known limits:
  - file-level evidence is not normalized and therefore remains `not_captured`
  - large project scopes are bounded to a newest Session/tool sample and expose
    that sampling in the report; T85's append-only import optimization is
    complete but remains process-local and source-boundary aware
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: evaluate one persisted Experiment across comparable Tasks with
  explicit Outcome and metric coverage, without converting process differences
  into a universal or causal quality verdict
- implemented scope:
  - added the Core `cohort-runtime-profile/v1` contract and builder with
    per-configuration task samples, mean/median/P90/min/max distributions for
    duration, total tokens, known cost, tool-error rate, peak context, and cache
    hit rate
  - enforced three Outcome-eligible Tasks per group and 50% metric coverage as
    minimum thresholds; missing Outcome fields remain in total samples but are
    excluded from guarded comparison
  - evaluated only bounded `{ metric, maxRelativeRegression }` guardrails;
    arbitrary persisted guardrail values remain `not_evaluable`
  - added `GET /api/experiments/:id/profile` through the Task repository, using
    Cohort definition matching and exactly one control/candidate Configuration
    Snapshot per comparable Task; no raw Task goal, prompt, or Outcome prose is
    returned
- acceptance result:
  - insufficient samples, missing Outcome, unsupported primary metrics, and
    incomplete coverage produce `insufficient_evidence` with explicit
    limitations
  - sufficient samples produce descriptive relative differences and bounded
    guardrail pass/fail states; the report has no winner field and never mutates
    persisted Experiment decisions or runtime configuration
  - no schema migration was needed; existing Task/Outcome/Experiment records
    remain the source of truth
- changed files:
  - `packages/core/src/cohort-runtime-profile.ts`, Core export, and focused
    distribution/guardrail tests
  - `apps/server/src/task-repository.ts`, Task route, and focused API/repository
    tests
  - `ARCHITECTURE.md`, `docs/profile-model.md`, `docs/tasks-outcomes.md`,
    `docs/agent-runtime-profile-design.md`, both READMEs, and roadmap
- verification:
  - Core 34 files / 218 tests passed; Server 27 files / 116 tests passed; Web
    12 files / 36 tests passed
  - `pnpm lint`, `pnpm build`, `pnpm check:boundaries`, and `git diff --check`
    passed
  - focused tests covered distribution rank statistics, Outcome-guarded sample
    eligibility, unsupported guardrails, insufficient evidence, repository
    aggregation, and the profile route
- known limits:
  - the current Cohort UI definition supports project, Task type, and
    complexity; broader time-window and statistical regression contracts remain
    future scope
  - Runtime feedback, automatic keep/rollback mutation, and universal Agent or
    configuration ranking remain intentionally out of scope
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: make evidence-sufficient T89 findings consumable after a completed
  Task without automatic Prompt/rule rewrites, live in-run control, or causal
  claims beyond the persisted Experiment decision contract
- implemented scope:
  - added the Core `post-run-feedback/v1` builder and contract with fixed
    suppression reasons, candidate-only findings, primary metric/guardrail
    summaries, Experiment/Cohort references, and explicit limitations
  - added opt-in read-only `GET /api/tasks/:id/feedback?optIn=true`; it derives
    reports from current Task/Outcome/Experiment records and the T89 Profile,
    without adding storage or copying raw Task/transcript content
  - emits no record for Tasks outside the current comparable sample, and
    suppresses feedback for incomplete Tasks, unverified Outcome, incomplete
    Experiments, insufficient/stale Profiles, absent keep/rollback decisions,
    and control baselines
  - added a Task workspace card that explicitly loads and presents available
    findings or the suppression/empty state; no configuration mutation is
    available from the finding
- acceptance result:
  - every emitted finding links to the current cohort evidence version/time,
    Experiment/Cohort IDs, metric/guardrail summaries, and limitations
  - opt-in and privacy boundaries are enforced at the API; no goal, acceptance
    prose, Prompt, rules, transcript, tool output, or chain-of-thought is
    returned or persisted by the feedback contract
  - stale or insufficient evidence produces suppression rather than a
    recommendation, and the report remains descriptive rather than causal
- changed files:
  - `packages/core/src/post-run-feedback.ts`, export, and focused Core tests
  - `apps/server/src/task-repository.ts`, Task feedback route, and Server tests
  - `apps/web/app/tasks/page.tsx`
  - `ARCHITECTURE.md`, `docs/profile-model.md`, `docs/tasks-outcomes.md`,
    `docs/agent-runtime-profile-design.md`, both READMEs, Chinese overview, and
    roadmap register/archive
- verification:
  - Core focused feedback tests passed; full Core suite passed: 36 files / 222
    tests
  - Server focused Task/API tests passed: 27 files / 116 tests
  - Web suite passed: 12 files / 36 tests; `pnpm lint`, full `pnpm build`,
    `pnpm check:roadmap`, `pnpm check:boundaries`, isolated temporary Server/Web
    smoke flow, and `git diff --check` passed
  - isolated browser flow created a temporary Task and confirmed the Task page
    rendered the post-run feedback card and empty-state copy; the temporary
    database and local services were not connected to the user database
  - required `codex review --uncommitted` could not initialize because the
    sandbox exposed `~/.codex/state_5.sqlite` as read-only; the prescribed
    manual audit covered tracked and untracked changes, fixed stale feedback
    after an Experiment decision update, and found no remaining CRITICAL/HIGH
    findings
- known limits:
  - feedback is derived on request and is not an independently persisted record;
    external Runtime SDK consumption and live in-run hints remain future work
  - the current Experiment/Cohort definition scope and T89 minimum evidence
    thresholds remain the comparison boundary

<a id="t109"></a>

### T109 unified Project Profile selector

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: replace the Project Profile native long-path selector with the same
  searchable, grouped project-picker interaction used on the Home workspace
- implemented scope:
  - added a shared project-summary adapter so Project Profile can reuse the
    existing picker while preserving short names, parent paths, session counts,
    search, and keyboard navigation
  - added a required-selection picker mode that omits the inapplicable “all
    projects” option and does not reference a missing active option when a
    search has no result; Home retains its existing all-projects filtering
    behavior
  - replaced the Stats Project Profile native `<select>` and added an explicit
    no-project empty state
- acceptance result:
  - Project Profile selection remains a read-only request to the existing
    project-profile API and does not change profile aggregation or storage
  - long paths are searchable, a mouse or keyboard selection loads the matching
    Project Profile, and light/dark themes reuse the established picker tokens
- changed files:
  - `apps/web/app/project-picker.tsx`, `apps/web/app/session-navigation.ts`,
    `apps/web/app/stats/page.tsx`, and `apps/web/app/session-navigation.test.ts`
  - roadmap register and archive
- verification:
  - Web suite passed: 12 files / 36 tests; `pnpm lint`, production Web build,
    `pnpm check:roadmap`, and `git diff --check` passed
  - browser verification against the local Stats data confirmed the grouped
    searchable list contains no all-projects choice; a long-path selection
    updated the Profile, and ArrowDown + Enter selected and loaded another
    project in both light and dark themes
  - `codex review --uncommitted` could not initialize because the sandbox
    exposes `~/.codex/state_5.sqlite` as read-only; the prescribed manual
    review covered the tracked diff, fixed the empty-search
    `aria-activedescendant` reference, and found no remaining CRITICAL/HIGH
    findings
...
- started_at: 2026-08-02
- completed_at: 2026-08-02
- implemented scope: persisted only Codex rollout `session_meta.parent_thread_id`
  as an additive, atomically replaced source-native child-to-parent link.
  Claude, Zed, MiMo, and OpenCode event-level parent IDs do not produce an
  inferred Session relationship.
- acceptance result: Session detail distinguishes linked, parent-unavailable,
  and not-captured coverage; reset and child replacement remove stale child
  links without touching annotations. Combined resource attribution remains
  deferred.
- documentation: updated architecture, statistics limitations, README, and
  roadmap register/archive with the Codex-only support boundary.
- verification: parser, repository atomic-replacement/reset, additive-migration,
  linked/unavailable API, and focused Web label tests passed; Server 26/110,
  Core 30/208, Web 11/34, and production Web build passed.
- changed files: Core Codex metadata; Server migration/repository/route/tests;
  Web relationship card/helper/tests; current-state documentation and roadmap.

<a id="t110"></a>

### T110 profile-evolution documentation and roadmap decomposition

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: reconcile the current profiler assessment across product, architecture,
  privacy, and runtime-design documents, then turn the verified gaps between
  post-hoc diagnosis and a bounded Agent feedback loop into an ordered backlog.
- implemented scope:
  - added proposal-only `docs/profile-evolution-plan.md`, with the current
    capability verdict, strengths/limits, contract rules, decision points,
    delivery sequence, and success-measure guardrails.
  - corrected stale current/future claims across product, architecture, diagnosis,
    Task/Outcome, multi-Agent, Model Catalog, English, and Chinese documents.
    This includes the implemented bounded cohort report, T100 Web workspace,
    T87 source-native relationship, 11 deterministic diagnosis rules, Outcome
    coverage semantics, source-observation boundary, semantic-provider payload,
    endpoint-locality boundary, and existing CLI scope.
  - added planned T111-T121 with independent purpose, scope, dependencies, risks,
    acceptance criteria, verification, documentation responsibilities, and a
    trust/evidence-first execution order.
- acceptance result:
  - current-state documents distinguish local evidence refresh from live Runtime
    tracing; Outcome coverage from verification status; bounded descriptive cohort
    comparison from causal ranking; source-native relationships from combined
    attribution; and explicit hint adoption from automatic configuration mutation.
  - semantic diagnosis now discloses its current bounded title/thinking/tool-input
    provider payload, lack of request-level consent/redaction/audit, default
    external endpoint, and the fact that custom endpoint locality is not verified.
  - proposal documents label target contracts as future work and do not claim
    automatic configuration mutation, live control, remote deployment, or causal
    configuration winners.
- changed files: `AGENTS.md`, `ARCHITECTURE.md`, `README.md`, `README.zh-CN.md`,
  `docs/profile-evolution-plan.md`, `docs/profile-model.md`,
  `docs/agent-runtime-profile-design.md`, `docs/diagnosis.md`,
  `docs/tasks-outcomes.md`, `docs/multi-agent.md`,
  `docs/model-configuration-design.md`, `docs/zh/OVERVIEW.md`, and the roadmap
  register/archive.
- verification:
  - manual cross-document review checked current behavior against architecture,
    CLI contracts, source-native relationship records, and diagnosis-provider
    construction; two independent read-only audits found no blocking discrepancy.
  - focused stale-claim searches found no old seven-rule count, T87/T100/T102
    future claim, obsolete Task API path, or automatic-hint edge.
  - after this terminal transition, `pnpm check:roadmap` passed with 12 current,
    86 archived, and 0 in-progress Tasks; `git diff --check` passed.
- known limits: this Task only documents and decomposes work. T111-T121 remain
  planned, so request-level semantic consent/redaction/audit, direct
  finding-to-evidence navigation, Outcome producers, Agent-facing workflow,
  Runtime events/hints, stronger comparison, combined attribution, price
  governance, and large-history virtualization are not implemented by T110.
...
- completed_at: 2026-08-03
- implemented scope:
  - changed the default diagnosis path to deterministic-only; semantic diagnosis
    requires the explicit request-scoped `semantic=opt_in` query value.
  - added bounded common-secret redaction for the Session title, thinking
    excerpts, tool names, and tool inputs before Provider payload construction.
  - added validated semantic status/payload metadata and a process-local audit
    store bounded to 100 entries; audit entries contain no source or Provider
    response content.
  - added Web disclosure and explicit opt-in action, preserving heuristic
    findings when Provider configuration, transport, or parsing fails.
- acceptance result: no Provider call occurs without opt-in; fake-Provider tests
  inspected the outgoing body and found no fixture secrets; returned span IDs are
  constrained to current evidence; deterministic analysis remains available
  without an LLM key; invalid semantic query values are rejected.
- changed files: Core diagnosis contract and shared redaction utility; Server
  semantic client, route, audit store, and tests; Session Web disclosure/action;
  README, Chinese README/overview, architecture, diagnosis, runtime proposal,
  profile-evolution plan, and roadmap documents.
- verification:
  - `pnpm --filter trace-server build` passed.
  - Server tests passed: 29 files / 122 tests.
  - Core tests passed: 36 files / 223 tests.
  - Web tests passed: 12 files / 38 tests; production Web build passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` passed.
  - A live Browser check could not start because no local Web service was
    listening on `127.0.0.1:3001`; fake-Provider payload inspection and Web
    production build were used instead.
- known limits: the redaction pass covers common credential forms and is not a
  guarantee against every secret; endpoint locality remains unverified; audit
  metadata is process-local and not a durable historical ledger.

<a id="t122"></a>

### T122 concise commit-message convention

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added a repository convention requiring a concise
  `<type>(<optional scope>): <imperative outcome>` subject, a 72-character
  subject limit, one logical Task per commit, prohibited vague subjects, and an
  optional body only for non-obvious constraints or verification context.
- acceptance result: the rule retains the explicit user-authorization and review
  gates, adds no hook, and this Task's selected subject is
  `docs: define commit message convention`.
- changed files: `AGENTS.md` and the roadmap register/archive.
- verification:
  - after the terminal transition, `pnpm check:roadmap` passed with 12 current,
    87 archived, and 0 in-progress Tasks; `git diff --check` passed.
  - pure-documentation exception applied to the commit review route: the staged
    diff was manually audited for scope and secrets; it contains no runtime code,
    dependency, schema, API, or behavior change.
- known limits: the convention is review-enforced; no local Git hook or CI rule
  was added.

<a id="t123"></a>

### T123 embedded Session relationship navigation containment

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added a validated same-origin embedded Session navigation
  message carrying the current and target Session IDs; Home accepts it only
  from the active iframe and current Session, then reuses the existing
  selection/history path. Parent and child relationship links intercept only
  unmodified primary clicks in embedded mode, while standalone links remain
  direct. Added top-level navigation fallbacks to Session detail, the global
  Header, 404, and compare return links so an accidental iframe load cannot
  create another nested Home page.
- acceptance result: the former child Session → “返回列表” recursion is
  contained; the outer URL and detail iframe move to the target child, the
  embedded child contains no nested iframe or standalone return link, and
  “返回总览” restores `/` with no detail iframe. Standalone Session detail
  returns to `/` with `target="_top"`.
- changed files: `apps/web/app/page.tsx`,
  `apps/web/app/session-detail-transition.ts`,
  `apps/web/app/session-detail-transition.test.ts`,
  `apps/web/app/session/[id]/page.tsx`,
  `apps/web/app/session/[id]/source-relationship-card.tsx`,
  `apps/web/app/header.tsx`, `apps/web/app/not-found.tsx`,
  `apps/web/app/compare/page.tsx`, `ARCHITECTURE.md`,
  `docs/ui-guidelines.md`, and the roadmap register.
- verification:
  - `pnpm --filter agent-profile-web test` — 12 files / 38 tests passed
  - `pnpm --filter agent-profile-web build` — passed with the dynamic
    `/session/[id]` route
  - `pnpm lint` — passed; `pnpm check:roadmap` — passed; `git diff --check` —
    passed
  - in-app browser against the existing local Web/API services — parent
    Session opened one iframe; relation navigation changed outer URL to the
    child while keeping one iframe, produced zero nested iframe and zero
    embedded “返回列表” links, and returning restored `/` with zero iframes;
    standalone detail exposed one `_top` return link and returned to `/`
- known limits: the browser check used the existing local services because a
  new `pnpm dev` instance was blocked by an occupied local port; no commit was
  created because the user did not request one.
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added exact, bounded finding-to-evidence navigation from
  Session diagnosis findings to the existing evidence page. Finding Span IDs
  are validated and capped by the Server route, carried in query-bound paging
  state, and preserved in a reloadable `view=evidence&spanIds=...` URL. The Web
  view keeps content hidden by default, preserves existing filters/paging, and
  reports missing or filter-excluded targets explicitly; findings without Span
  IDs show an unavailable reason.
- acceptance result: findings with stored Span references open the focused
  evidence query; the query remains `content=none` by default; target IDs do not
  substitute nearby events; exact-target URL parsing, route filtering, invalid
  IDs, and clear-focus behavior are covered by tests/build validation.
- changed files: Core evidence query contract; Server evidence service, route,
  and tests; Session Web URL state, diagnosis actions, focused evidence panel,
  and helper tests; README, Chinese README/overview, architecture, diagnosis,
  UI guidance, profile evolution, runtime proposal, and roadmap documents.
- verification:
  - `pnpm --filter trace-server build` passed.
  - Server tests passed: 29 files / 123 tests.
  - Web tests passed: 12 files / 39 tests; production Web build passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` passed.
  - A live Browser check was not run because no local Web service was listening
    on `127.0.0.1:3001`; route/helper tests and the production build covered the
    focused query and URL behavior.
- known limits: a target can become unavailable after a rebuild or active
  filter; the feature reports that state and does not infer replacement
  evidence. The exact target list is bounded to 20 IDs per request.
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added `task-assistance/v1` for bounded local Task
  assistance. The Task workspace can explicitly prefill a new Task's title and
  project from a locally observed Session, review same-project/time-window
  Session candidates, and review local Git commit candidates. Each suggestion
  is independently ignored or accepted; accepted Session links retain producer,
  capture time, source ID, and correlation basis, while accepted Git references
  stay in the unsaved Outcome draft until the user saves it.
- acceptance result: no candidate is silently attached or persisted; no Git
  candidate supplies build/test/lint/rating success; Task Profile coverage keeps
  the existing five-field semantics; local candidate responses contain bounded
  metadata and omit prompt/transcript content; invalid provenance is rejected.
- changed files: Core Task evidence provenance and `task-assistance/v1` contract;
  Server migration v11, Task repository validation/storage, assistance report,
  route, and local Git fixture tests; Task Web prefill, candidate review, and
  provenance-preserving Outcome draft; README, profile model, Task/Outcome
  design, architecture, Chinese overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/core test` — 36 files / 223 tests passed.
  - `pnpm --filter trace-server test -- --run src/__tests__/tasks.test.ts src/__tests__/database.test.ts` — 29 files / 124 tests passed.
  - `pnpm --filter trace-server build` — passed.
  - `pnpm --filter agent-profile-web test` — 12 files / 39 tests passed.
  - `pnpm --filter agent-profile-web build` — passed; `/tasks` remained a static route.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` — passed.
- known limits: correlation uses the Task project key and a seven-day local
  time window, limits Session/Git results, and does not prove membership or
  delivery quality. Git lookup is local-only, bounded, and unavailable
  repositories are reported as limitations; remote CI and arbitrary command
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added the versioned `outcome-evidence/v1` contract with
  explicit producer, capture time, source, bounded records, capture limits, and
  limitations. Added the approved read-only local Git adapter and
  `GET /api/tasks/:id/outcome-evidence?source=local_git`; it reads only fixed
  Git metadata from a linked Session cwd or absolute Task project and does not
  write Outcome data. Extended evidence statuses with `not_captured` and
  `observed` while preserving the five-field `task-profile/v1` coverage
  denominator.
- acceptance result: reports distinguish `not_captured`, `observed`, `passed`,
  `failed`, `skipped`, and `not_run`; provenance is visible; valid Git fixtures
  return bounded observed metadata; missing cwd or unavailable Git returns
  `not_captured` with limitations; arbitrary commands, uploads, remote CI, and
  automatic Outcome writes remain outside the boundary.
- changed files: `packages/core/src/outcome-evidence.ts`,
  `packages/core/src/task-profile.ts`, `packages/core/src/index.ts`,
  `apps/server/src/local-git-outcome-adapter.ts`,
  `apps/server/src/__tests__/outcome-evidence.test.ts`,
  `apps/server/src/routes/tasks.ts`, `apps/server/src/task-repository.ts`,
  `apps/web/app/tasks/outcome-model.ts`,
  `apps/web/app/tasks/outcome-editor.tsx`, and the current-state Task/Outcome,
  profile, architecture, README, Chinese overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/core test` — 36 files / 223 tests passed.
  - `pnpm --filter trace-server test -- --run src/__tests__/outcome-evidence.test.ts src/__tests__/tasks.test.ts src/__tests__/database.test.ts` — 30 files / 127 tests passed.
  - `pnpm --filter agent-profile-web test` — 12 files / 39 tests passed.
  - `pnpm --filter trace-server build` and
    `pnpm --filter agent-profile-web build` — passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` — passed.
  - `codex review --uncommitted` could not initialize because the environment
    denied the Codex state DB/app-server; the required manual review fallback
    found no CRITICAL/HIGH findings.
- known limits: the approved adapter observes Git metadata only; it does not
  prove build/test/lint success or delivery quality, and remote evidence
  connectors remain deferred to later Tasks.
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added `agent-profile-cli/v1` commands for bounded local
  inspection and explicit writing: `diagnosis <session-id>`,
  `evidence <session-id>`, `task-outcome <task-id> --confirm`, and
  `task-feedback <task-id> --opt-in`. Added Server report builders for
  content-free diagnosis/evidence references, validated Outcome evidence
  append, and bounded post-run feedback consumption. Diagnosis/evidence input
  is capped and omits raw process content; Outcome writes reuse TaskRepository
  validation and do not infer passing checks.
- acceptance result: an Agent can complete
  `inspect -> cite evidence -> record confirmed Outcome -> read bounded
  feedback` through versioned local interfaces. Confirmation and opt-in gates
  are explicit; no command exposes prompt/answer/thinking/tool content by
  default, mutates Agent configuration, or claims delivery success from process
  metrics.
- changed files: `packages/contracts/src/cli.ts`,
  `packages/contracts/src/index.ts`, `packages/cli/src/runner.ts`,
  `packages/cli/src/main.ts`, `packages/cli/src/runner.test.ts`,
  `apps/server/src/reports-service.ts`,
  `apps/server/src/__tests__/local-reports.test.ts`, and CLI,
  architecture, Task/Outcome, profile, runtime-proposal, README, Chinese
  overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/contracts build` — passed.
  - `pnpm --filter @agent-profile/cli test` — 4 files / 29 tests passed.
  - `pnpm --filter @agent-profile/cli build` and
    `pnpm --filter trace-server build` — passed.
  - `pnpm --filter trace-server test -- --run src/__tests__/local-reports.test.ts` — 31 files / 129 tests passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` — passed.
  - `codex review --uncommitted` could not initialize because the environment
    denied the Codex state DB/app-server; the required manual review fallback
    found no CRITICAL/HIGH findings.
- known limits: this is a loopback/local CLI workflow, not a remote control
  plane or live Runtime protocol. Diagnosis and evidence reports are bounded;
  raw content and full historical paging remain in the Web/API surfaces.
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added the framework-neutral `runtime-event/v1`,
  `runtime-event-batch/v1`, and `runtime-event-page/v1` contracts for bounded
  Task/Run lifecycle metadata. Added migration v12 and a loopback collector with
  `POST /api/runtime/events` and bounded sequence-ordered
  `GET /api/runtime/runs/:runId/events` references. The collector validates
  identity, event kind, sequence, timestamps, parent references, and an
  allowlisted metadata payload; raw prompt, answer, thinking, tool input, and
  tool output fields are rejected. Exact duplicates are idempotent, event ID
  and sequence conflicts are isolated as rejected input, out-of-order arrivals
  are retained with ordering coverage, and stored pages omit payload values.
- acceptance result: local lifecycle capture is source-neutral, bounded, and
  independent of transcript import. Batch and page limits are enforced,
  malformed or private-content input does not partially write, failures are
  surfaced explicitly, duplicate/conflict behavior is deterministic, and
  complete/partial/not-captured coverage is exposed. No live hint, prompt
  mutation, configuration mutation, remote ingestion, or transcript
  replacement was added.
- changed files: `packages/contracts/src/runtime-events.ts`,
  `packages/contracts/src/index.ts`, `apps/server/src/database.ts`,
  `apps/server/src/runtime-event-collector.ts`,
  `apps/server/src/routes/runtime-events.ts`,
  `apps/server/src/routes/index.ts`, runtime collector/route tests,
  `apps/server/src/__tests__/database.test.ts`, and the current-state
  architecture, README, profile model, runtime proposal, evolution plan,
  Chinese overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/contracts build` — passed.
  - `pnpm --filter trace-server test` — 33 files / 133 tests passed.
  - `pnpm --filter trace-server build` — passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` — passed.
  - `codex review --uncommitted` could not initialize because the environment
    denied the Codex state DB/app-server; the required manual review fallback
    found no CRITICAL/HIGH findings. The collector append function is longer
    than the review checklist preference but is non-blocking.
- known limits: the collector is loopback/local and metadata-only; events may
  be unavailable, partial, or out of order, and the page currently uses a
  bounded limit without a cursor. It is an observed process-evidence source,
  not a complete transcript, delivery proof, live control channel, or causal
  configuration evaluator. T117 owns any future in-run hint policy.
...
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added the pure `runtime-hint/v1` policy builder with
  explicit opt-in, fresh/partial/not-captured event coverage, five-minute
  freshness, future-timestamp and finished-Run suppression, short expiry,
  per-Run rate limiting, confidence, bounded event and historical evidence
  references, and fixed suppression reasons. The policy emits only a
  repeated-tool-failure hypothesis after a ready descriptive Cohort profile is
  available; it never emits a configuration mutation or a delivery verdict.
  Added migrations v13/v14 for bounded hint/adoption records and durable
  rejected-event/coverage-attestation state, the local hint and explicit
  adoption routes, and repository lookup that requires an unambiguous current
  Task/Experiment match. Every Runtime batch must explicitly attest
  `coverageComplete: true`; missing or false attestation suppresses hints.
- acceptance result: `GET /api/runtime/runs/:runId/hint?optIn=true` returns an
  available short-lived content-free hint only when all evidence gates pass;
  otherwise it returns a structured suppression reason. Hint payloads contain
  no prompt, answer, thinking, tool input, or tool output content. Adoption is
  accepted only through `POST /api/runtime/hints/:hintId/adoption` with explicit
  `adopted`, `ignored`, or `not_recorded` status and a bounded producer; later
  Runtime behavior never infers adoption. No endpoint changes Agent prompts,
  rules, tools, models, or configuration.
- changed files: `packages/core/src/runtime-hints.ts`, Core export and policy
  tests; migration/database tests; Runtime signal extraction; historical
  evidence lookup in `apps/server/src/task-repository.ts`; the hint service,
  routes, and integration tests; and current-state architecture, README,
  profile model, Task/Outcome, runtime proposal, evolution plan, Chinese
  overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/core test` — 38 files / 232 tests passed.
  - `pnpm --filter trace-server test` — 34 files / 141 tests passed.
  - `pnpm --filter @agent-profile/core build` and
    `pnpm --filter trace-server build` — passed.
  - `pnpm lint`, `pnpm check:roadmap`, and `git diff --check` — passed.
  - focused Core policy tests cover opt-in, freshness, future timestamps,
    finished Runs, partial coverage, rate limiting, and signal gating; focused
    Server tests cover migrations, event identity/coverage, Task-scoped history,
    hint routes, and explicit adoption.
  - `codex review --uncommitted` could not initialize because the environment
    denied the Codex state DB/app-server; the required manual review fallback
    found no CRITICAL/HIGH findings. Non-blocking review notes were fixed by
    suppressing future-dated/finished-Run hints and preserving rejected-event
    coverage, and aligning append coverage status with the attestation state;
    rebuilding bounded historical profiles per eligible experiment remains a
    non-blocking maintainability note.
- known limits: this is a loopback/local policy, not an external Runtime SDK;
  it currently covers repeated tool failures only, requires a ready historical
  comparison, and uses a bounded latest-event window without cursor paging.
  Hints are hypotheses with descriptive evidence, not causal recommendations,
  quality verdicts, or automatic remediation. T118 owns broader comparison
  rigor and future hint categories.

<a id="t124"></a>

### T124 runtime-mode model-label classification

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: prevent Codex runtime-mode labels such as `codex-auto-review` from
  being represented as concrete LLM models while retaining exact, explicitly
  known model-ID recognition
- implemented scope:
  - added a distinct `runtime_mode` presentation identity and classified
    `codex-auto-review` under it; the raw Span `model` value remains unchanged
  - expanded explicit concrete-ID recognition to captured Codex IDs and bundled
    configured model IDs without heuristic model-name guessing
  - excluded known runtime-mode values from model statistics and Model Catalog
    inventory, and rejected their pricing, context, and alias writes
  - exported the runtime-mode predicate and updated the CLI statistics contract
- acceptance result: `codex-auto-review` is neither shown as a model nor
  configurable as one; recognized concrete IDs continue to use canonical model
  identities; unknown source values remain observable and unpriced
- documentation: updated `ARCHITECTURE.md`, `docs/stats.md`, and the roadmap
  with the source-evidence, exclusion, and configuration boundary
- verification: focused Core identity and Server statistics/Model Catalog tests
  passed; full test, build, lint, roadmap, and diff checks remain to be rerun
  after this rebase
- changed files: Core identity/export/tests; Server Model Catalog and statistics
  implementation/tests; CLI statistics contract; current-state documentation

<a id="t125"></a>

### T125 rebase main and resolve upstream conflicts

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- purpose: reconcile the local `main` branch with the latest `origin/main`
  while preserving the intended Project Profile and runtime-mode behavior
- implemented scope:
  - fetched `origin/main` and rebased the two local commits onto it, producing
    `804504c` and `9ff6aae`
  - resolved duplicate Project Profile implementations in favor of the current
    upstream `project-profile/v1` Core/API contract, retained the local `/projects`
    page, and added a Web-only response normalizer for that upstream schema
  - removed an automatically duplicated Project Profile route registration and
    preserved upstream roadmap history; renamed the colliding runtime-mode task
    to T124 and used T125 for this rebase task
- acceptance result: `main` is based on `origin/main`, no unmerged paths remain,
  Project Profile and runtime-mode behavior are preserved, and the resulting
  changes are committed locally
- changed files: Project Profile Web adapter/page/test; duplicate route
  registration; English/Chinese README, stats, and roadmap documentation;
  runtime-mode Core/Server/CLI code, tests, and documentation
- verification:
  - `git fetch origin`, `git rebase origin/main`, and final branch inspection
    passed; `main` is ahead 2 and behind 0 relative to `origin/main`
  - `pnpm --filter @agent-profile/core test` — 36 files / 226 tests passed
  - `pnpm --filter trace-server test` — 33 files / 134 tests passed
- `pnpm --filter agent-profile-web test` — 13 files / 41 tests passed
- `pnpm build` and `pnpm lint` passed
- `pnpm check:roadmap` and `git diff --check` passed
- `codex review --base origin/main` could not initialize because the environment
  denied the Codex state DB/app-server; the required manual review fallback
  found no CRITICAL/HIGH findings
- known limits: no remote push was performed; the pre-rebase T91 stash remains
  as a local backup because T91 is already an upstream Task ID

<a id="t126"></a>

### T126 persistent code-review workflow rules

- status: completed
- started_at: 2026-08-03
- completed_at: 2026-08-03
- implemented scope: added a repository-scoped Code Review Workflow to
  `AGENTS.md`. It documents that ordinary natural-language requests trigger
  review without requiring a CLI command, requires inspection of staged,
  unstaged, and untracked files, keeps review passes read-only, defines the
  Codex CLI route and manual fallback, and records severity and rerun rules.
- acceptance result: the project rule now distinguishes the user-facing review
  request from the internal execution command; it prevents review-only passes
  from editing, staging, committing, or pushing, and requires the same route to
  be rerun after blocking findings are fixed.
- changed files: `AGENTS.md`, `docs/roadmap.md`, and this archive entry.
- verification:
  - final `AGENTS.md` rule text inspected.
  - `pnpm check:roadmap` — passed.
- known limits: the rule is scoped to this repository and does not change the
  availability or permissions of the Codex review CLI.
...
- implemented scope: added declared comparability strata for `project_id`,
  `task_type`, and `complexity`, with configurable minimum eligible Tasks and
  Outcome coverage. Strata with missing declared values, without a
  control/candidate counterpart, or with insufficient Outcome evidence are
  excluded with visible reasons and Task IDs. Extended
  `cohort-runtime-profile/v1` with `ready`, `insufficient_evidence`, and
  `not_comparable` states, comparable/excluded sample counts, median/p25/p75/p90,
  sample standard deviation, absolute/relative effects, and a bounded 95%
  normal-approximation relative-effect interval. TaskRepository now reads the
  optional `comparability` declaration from the Cohort definition and passes
  content-free Task strata to the Core builder.
- acceptance result: reports identify the declared strata and exclusions,
  distinguish Outcome coverage from favorable status, expose uncertainty and
  limitations, preserve explicit user-owned keep/rollback decisions, and never
  emit a universal configuration winner. No prompt or transcript content is
  used to infer comparability.
- changed files: Core cohort profile types/builder/tests; TaskRepository and
  Task integration tests; current-state README, architecture, profile model,
  Task/Outcome, Runtime proposal, Chinese overview, and roadmap documents.
- verification:
  - `pnpm --filter @agent-profile/core test` — 38 files / 237 tests passed.
  - `pnpm --filter trace-server test` — 34 files / 142 tests passed.
  - Core and Server TypeScript checks/builds — passed.
  - `pnpm lint`, `pnpm check:boundaries`, `pnpm check:roadmap`, and
    `git diff --check` — passed.
  - `codex review --uncommitted` stalled during CodeGraph/MCP initialization
    in the read-only environment; the required manual diff audit found no
    CRITICAL/HIGH (P0/P1) findings.
  - focused fixtures cover ready, insufficient evidence, not comparable,
    excluded strata, distributions, guardrails, and uncertainty.
- known limits: uncertainty uses a bounded normal approximation rather than a
  causal or bootstrap conclusion; stronger regression policy, repeated-measure
  controls, and automatic decisions remain future work.

<a id="t119"></a>

### T119 multi-Agent Task graph and non-double-counted attribution

- status: completed
- started_at: 2026-08-07
- completed_at: 2026-08-07
- implemented scope: made the Task the explicit multi-Agent delivery boundary
  in `task-profile/v1`. Added a typed Task graph whose nodes are the linked
  Sessions (link role, Agent, availability); edges come only from stored
  source-native `session_relationships` rows (relation kind `source_parent`)
  that touch a linked Session, naming the source and whether the counterpart
  Session row is stored and linked to the same Task. Per-Agent attribution
  counts linked/available Sessions and sums tokens, cost, unknown-cost count,
  tool calls, and tool errors once per stored linked Session. Relationship
  coverage reports captured edges, partial edges (counterpart unavailable or
  unlinked), and absent linked Sessions with no stored relationship. Aggregate
  totals reconcile exactly to the linked stored Sessions; a child Session
  linked to the Task is never summed a second time through a parent edge.
  `TaskRepository.buildProfile` now loads relationships for the linked Session
  set in one query and passes them to the Core builder. The Web Task workspace
  shows per-Agent attribution and relationship coverage in the Task Profile
  card.
- acceptance result: `task-profile/v1` exposes nodes, edges, per-Agent
  attribution, and relationship coverage; totals reconcile to the linked stored
  Sessions without double counting; missing or unavailable relationships are
  reported as coverage limitations and never replaced by an inferred edge from
  titles, paths, timestamps, or model similarity; the change is additive to
  `task-profile/v1` and no schema migration was required (edges reuse the
  existing `session_relationships` table).
- changed files:
  - Core: `packages/core/src/task-profile.ts` (graph types and builder),
    `packages/core/src/index.ts` (exports), and
    `packages/core/src/__tests__/task-profile.test.ts`.
  - Server: `apps/server/src/task-repository.ts` (relationship loading) and
    `apps/server/src/__tests__/tasks.test.ts`.
  - Web: `apps/web/app/tasks/page.tsx` (Task Profile graph summary).
  - Documentation: `ARCHITECTURE.md`, `docs/tasks-outcomes.md`,
    `docs/multi-agent.md`, `docs/profile-model.md`,
    `docs/profile-evolution-plan.md`, `docs/zh/OVERVIEW.md`, and this
    roadmap register.
- verification:
  - `pnpm --filter @agent-profile/core test` — 38 files / 246 tests passed.
  - `pnpm --filter trace-server test` — 34 files / 148 tests passed.
  - `pnpm build` — Core, Server, Web, and CLI builds passed.
  - `pnpm lint`, `pnpm check:boundaries`, and `pnpm check:roadmap` — passed.
  - focused tests cover a flat Task without relationships (absent coverage), a
    linked parent + linked child (double-count guard), an unavailable or
    unlinked counterpart (partial coverage), and multi-Agent attribution
    reconciliation.
- known limits: graph edges describe stored source-native relationships only;
  a linked child Session whose parent is absent or unlinked remains a
  coverage limitation rather than merged evidence. Inferred parentage from
  text, paths, time, or model similarity remains out of scope.

<a id="t127"></a>

### T127 explicit push ownership and descriptive commit bodies

- status: completed
- started_at: 2026-08-04
- completed_at: 2026-08-04
- implemented scope: updated the repository commit convention and the global
  Codex preference so push is user-owned, failed pushes are reported without
  blind retries, and every commit has a useful subject plus body covering
  intent, material scope, and verification. Existing published commits were
  not rewritten.
- acceptance result: future commit handoffs stop after commit and leave push to
  the user; commit messages are required to carry readable context instead of
  subject-only summaries.
- changed files: `AGENTS.md`, `docs/roadmap.md`, and this archive entry;
  global preference in `~/.codex/AGENTS.md`.
- verification:
  - inspected repository and global rule text.
  - `pnpm check:roadmap` and `git diff --check` — passed.
- known limits: the global preference is outside repository history and may be
  regenerated only if the Codex rule adapter replaces the user section.
...
- cancelled_at: 2026-08-06
- decision: removed from the active roadmap because the implemented local,
  CNY-only Model Catalog already preserves pricing provenance and unknown-price
  behavior, while no current consumer requires multi-currency or remote price
  synchronization
- retained boundary: automatic scraping, silent currency conversion, and
  unreviewed historical price mutation remain out of scope; a future concrete
  requirement must create a new Task with its own consumer, trust, and migration
  decisions
- changed files: roadmap, Model Catalog design, architecture/product boundary,
  and evolution-plan documentation under T128
- verification: recorded by T128 after documentation reconciliation
...
- implemented scope: reconciled current-state product, architecture, Profile,
  performance, security, Model Catalog, Chinese overview, and proposal documents;
  added one cohesive set of runtime composition, module direction, ingestion,
  evidence/Task/feedback, security, and multi-agent boundary views; removed
  duplicate Project Profile definitions and stale claims that implemented local
  CLI evidence/Outcome workflows were future work
- decisions: Agent Profile remains one local-first modular application rather
  than a group of services; T88 option A is settled as loopback-only and retains
  only an enforcement gap; T120 is cancelled because there is no current
  multi-currency or remote-price consumer; proposal documents remain explicitly
  separate from current behavior
- acceptance result: architecture views agree with implementation, package
  boundaries do not imply microservices or multiple product CLIs, Profile terms
  have one canonical definition, current/proposed boundaries and documentation
  ownership are explicit, and execution order contains only open Tasks
- changed files: README and Chinese README; ARCHITECTURE; Profile model,
  evolution plan, Runtime proposal, Model Catalog design, performance guide,
  Chinese overview, roadmap, and this archive
- verification:
  - documentation consistency scans for stale T88/T120, duplicate Profile, and
    obsolete CLI/future-work claims — passed
  - `pnpm check:roadmap` — passed
  - `git diff --check` — passed
- known limits: T88 still needs its separate code change to reject non-loopback
  source-workspace Server hosts; T121 owns browser/Task-workflow scale budgets
...
- started_at: 2026-08-06
- completed_at: 2026-08-06
- implemented scope: the source-workspace Server now rejects non-loopback
  `HOST` values at startup instead of silently accepting them; the CLI already
  rejected non-loopback `--host`; CORS/origin guidance and the loopback-only
  boundary are aligned across README, Chinese README, ARCHITECTURE, Chinese
  overview, and the profile evolution plan
- decisions: option A (loopback-only) remains settled; non-loopback `HOST`
  values throw a startup configuration error naming the allowed loopback
  aliases; semantic provider egress stays a separate explicit opt-in content
  boundary
- acceptance result: Server and `agent-profile serve` reject non-loopback
  hosts; no user-facing documentation suggests exposing the unauthenticated API
  on a trusted network; local development and release startup remain unchanged
- changed files: `apps/server/src/config.ts`, `apps/server/src/index.ts`,
  `apps/server/src/__tests__/config.test.ts`, README, README.zh-CN,
  ARCHITECTURE, Chinese overview, profile evolution plan, roadmap, and this
  archive
- verification:
  - `pnpm --filter trace-server test` — 34 files / 143 passed, plus one new
    negative configuration case (144 total)
  - `pnpm --filter @agent-profile/cli test` — 4 files / 29 passed
  - `pnpm build`, `pnpm lint`, `pnpm check:roadmap`, `pnpm check:boundaries` —
    all passed
  - documentation consistency scan for stale `HOST` escape-hatch claims —
    passed
- known limits: none for this Task; T121 still owns browser/Task-workflow scale
  budgets
...
- started_at: 2026-08-06
- completed_at: 2026-08-06
- implemented scope: added Task/Cohort/Experiment/Configuration mutation DTOs
  and JSON-schema definitions to `@agent-profile/contracts`; added Scan/Import/
  Reset request DTOs and schemas; wired runtime JSON-schema validation into all
  Server mutation routes (tasks, config-snapshots, cohorts, experiments,
  imports, scan, data-management reset); replaced Web-local duplicate
  Task/Configuration/Cohort/Experiment interfaces and experiment-guardrail
  status types with contracts types; extended the module boundary checker to
  reject Web/CLI imports of Server application code and non-type contracts
  imports from Web
- decisions: Fastify 5 default type coercion is preserved, so scalar payloads
  that coerce into valid values remain accepted while structurally invalid
  payloads fail with 400; mutation schemas live with their DTOs in contracts
  rather than in route files
- acceptance result: public API shapes have one owner in contracts; malformed
  mutation payloads fail predictably with 400; boundary checks cover Web→Server
  and Web→contracts-value dependency directions
- changed files: `packages/contracts/src/tasks.ts` (new), `common.ts`,
  `index.ts`; `apps/server/src/routes/tasks.ts`, `scan.ts`; `apps/web/app/
  tasks/page.tsx`, `experiment-guardrail.ts`; `scripts/check-module-boundaries.
  mjs`; tasks/import route tests; ARCHITECTURE, roadmap, and this archive
- verification:
  - `pnpm --filter trace-server test` — 34 files / 146 passed (new malformed
    payload cases for tasks and scan/import/reset routes)
  - `pnpm --filter @agent-profile/cli test` — 4 files / 30 passed
  - `pnpm --filter agent-profile-web exec tsc --noEmit`, `pnpm build`,
    `pnpm lint`, `pnpm check:boundaries`, `pnpm check:roadmap` — all passed
- known limits: Fastify coercion means some scalar-typed payloads are accepted
  after conversion; this preserves pre-existing behavior and is documented in
  the route tests
...
- started_at: 2026-08-06
- completed_at: 2026-08-06
- implemented scope: added a content-free GitHub Actions workflow
  (`.github/workflows/ci.yml`) that runs lint, roadmap, module-boundary checks,
  all package tests, the production build, and the scale benchmark on push and
  pull request; added a bounded Playwright smoke suite under `apps/web/e2e`
  that starts the local CLI serve with disposable data and verifies the health
  endpoint and primary Web navigation, running as a separate CI job with
  Chromium installed
- decisions: CI stays content-free and never processes local transcript
  content or uploads evidence; smoke tests use a temporary data directory and
  `AUTO_SCAN_DIR=""` so they are deterministic and disposable
- acceptance result: CI is deterministic, content-free, and documented in
  README and ARCHITECTURE; smoke tests run against disposable local data and
  fail with actionable Playwright diagnostics
- changed files: `.github/workflows/ci.yml` (new), `apps/web/playwright.config.
  ts` (new), `apps/web/e2e/smoke.spec.ts` (new), `apps/web/vitest.config.ts`
  (new, excludes e2e from unit tests), `apps/web/package.json`
  (`@playwright/test` devDependency and `test:e2e` script), `.gitignore`,
  pnpm-lock.yaml, README, ARCHITECTURE, roadmap, and this archive
- verification:
  - `pnpm test` — core 238, web 41, server 146, CLI 30 (all passed)
  - `pnpm lint`, `pnpm check:roadmap`, `pnpm check:boundaries`, `pnpm build` —
    all passed
  - `pnpm --filter agent-profile-web test:e2e` — 2 Playwright smoke tests
    passed locally with Chromium
- known limits: the smoke suite requires a one-time Chromium install and a CLI
  build before running; CI installs both automatically
...
- started_at: 2026-08-07
- completed_at: 2026-08-07
- implemented scope: split oversized orchestration files into stable
  feature-local modules with a single clear owner. CLI: report formatting into
  `format.ts` and argument parsing into `arguments.ts` (runner.ts 1047 → 539
  lines). Server: task validation helpers into `task-validation.ts` and row
  mappers into `task-mappers.ts` (task-repository 1159 → 846 lines); database
  schema and migrations into `schema.ts` (database.ts 674 → 69 lines). Web:
  Session-detail analysis panels, ContextChart, diagnosis panels, and
  remaining view/misc panels into feature-local modules
  (session/[id]/page.tsx 2054 → 795 lines); home session-list UI and data
  management dialog (app/page.tsx 1206 → 903 lines); stats charts
  (stats/page.tsx 675 → 314 lines); model catalog editor
  (settings/models/page.tsx 707 → 258 lines); global styles into global.css
  (layout.tsx 1496 → 33 lines)
- exceptions recorded: `apps/web/app/tasks/page.tsx` (1029 lines) stays as a
  single inline-dialog page because its JSX blocks are tightly coupled to
  page-level state and extracting them would require passing a large prop
  surface with low cohesion gain; `task-repository.ts` (846 lines) stays as a
  single TaskRepository class because it is one complete responsibility unit
  and further splitting would fragment the repository contract;
  `model-catalog/service.ts` (705 lines) is under the guideline
- acceptance result: duplicated concepts have one owner; extracted units
  improve cohesion and keep existing tests green; CLI remains one command;
  priority files moved toward the 800-line guideline with documented
  exceptions
- changed files: 20+ feature-local modules across CLI, Server, and Web plus
  the affected page files, tests, roadmap, and this archive
- verification:
  - `pnpm test` — core 238, web 41, server 146, CLI 30 (all passed after each
    extraction batch)
  - `pnpm build`, `pnpm lint`, `pnpm check:roadmap`, `pnpm check:boundaries` —
    all passed after each batch
  - `pnpm --filter agent-profile-web test:e2e` — 2 Playwright smoke tests
    passed after each Web extraction
- known limits: the three recorded exceptions remain over or near the
  guideline with explicit rationale; T121 owns browser/Task-workflow scale
  budgets
...
- verification plan: focused Web tests, production build, browser checks at
  desktop and mobile widths, scale benchmark extension, and response-size
  regression checks
- documentation plan: update performance, architecture, README, UI guidance,
  and roadmap
- implemented behavior:
  - the Task workspace no longer loads the compatibility full Session array;
    `loadBase` fetches only Tasks, configurations, cohorts, and experiments
  - the two Session pickers (new-Task prefill and Session attachment) query
    bounded `session-discovery/v2` with a 50-row window, project/Agent/ID text
    search, keyset cursor, and explicit exclusion of already-linked IDs
  - added `apps/web/app/tasks/task-session-search.ts` (bounded search helper +
    unit tests) and `apps/web/app/tasks/session-picker.tsx` (debounced,
    cursor-paged picker shared by both surfaces)
  - extended `scale-benchmark` with the `taskSessionSearch` workload
    (`/api/session-discovery?limit=50&q=fixture`)
- measured result:
  - representative 500-Session fixture: compatibility `/api/sessions` full
    array 324,418 bytes at 2.6 ms median versus Task picker bounded 50-row
    discovery search 30,470 bytes at 2.3 ms median (~91% smaller transfer with
    stable keyset pagination)
  - `pnpm benchmark:scale:ci` passed with zero budget failures; whole-process
    maximum RSS 312,655,872 bytes
- bounded-list rationale: the Task workspace's remaining detail lists are
  bounded by design — linked Sessions come from the Task detail route (one row
  per explicit link), local assistance candidates are capped at 20, and
  cohort/experiment/feedback records are explicit definition rows; no unbounded
  rendered list remains after the Session picker migration, so virtualization
  was not required; the acceptance item "detail lists have stable
  virtual/window behavior" is satisfied by bounded data plus capped
  assistance/feedback surfaces plus the mobile-width overflow smoke check
- actual changed files:
  - Web: `apps/web/app/tasks/page.tsx`, `apps/web/app/tasks/session-picker.tsx`,
    `apps/web/app/tasks/task-session-search.ts`, and
    `apps/web/app/tasks/task-session-search.test.ts`
  - Server benchmark: `apps/server/src/performance/scale-benchmark.ts`
  - E2E: `apps/web/e2e/smoke.spec.ts`
  - documentation: `README.md`, `ARCHITECTURE.md`, `docs/performance.md`,
    `docs/zh/OVERVIEW.md`, and this archive
- verification:
  - `pnpm --filter agent-profile-web test` — 14 files / 44 tests passed
  - `pnpm --filter trace-server test` — 34 files / 146 tests passed
  - `pnpm --filter @agent-profile/core test` — 38 files / 238 tests passed
  - `pnpm lint`, `pnpm build`, `pnpm check:roadmap` — all passed
  - `pnpm --filter agent-profile-web test:e2e` — 4 Playwright smoke tests
    passed (bounded discovery contract, Task page never requests `/api/sessions`,
    mobile-width overflow check)
  - `pnpm benchmark:scale:ci` — passed with zero budget failures
- known limits: the browser smoke coverage uses an empty database, so the
  picker interaction with live Session data is covered by the bounded API
  contract and unit tests rather than a populated E2E fixture; the compatibility
  `/api/sessions` full-array route remains available and is still measured as a
  regression baseline
...
- implemented scope:
  - `webFilters` now accepts an excluded dimension; the Agent facet is
    computed with every active filter except the Agent dimension, and the
    Project facet with every active filter except the Project dimension;
  - both facet calculations retain time, text, quick-view, source-availability,
    and primary-Session predicates, and run inside the anomaly CTE so
    quick-view predicates stay consistent with the list;
  - facet counts are computed in SQLite with bounded indexed predicates; no
    client-side full-history reconstruction is introduced.
- decisions:
  - a facet count is a scope statement, not a global popularity measure; a
    selected dimension never hides a zero-result state.
- acceptance result: choosing `agent-profile` and Codex shows the actual
  intersection everywhere; switching either filter updates the other facet's
  counts deterministically.
- changed files: `apps/server/src/session-discovery-service.ts`,
  `apps/server/src/__tests__/session-discovery-service.test.ts`,
  `ARCHITECTURE.md`, `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 260 passed
  - `pnpm --filter trace-server test` — 159 passed
  - `pnpm --filter agent-profile-web test` — 45 passed
- known limits: Web navigation tests and desktop/narrow-width manual checks
  remain covered by the existing UI suite; a scale query-plan validation is
  covered by the bounded SQLite predicates and the existing benchmark suite.
...
- implemented scope:
  - added evidence-safe absence helpers (`session-absence.ts`) that render
    `not_captured` cost status as unavailable instead of `¥0`, `excluded` as
    excluded, `partial` as a known subtotal with incomplete-coverage warning,
    and `unknown`/`unknown_pricing` as unlabeled;
  - context cards show unavailable when the Session has no observable LLM
    turn; cache metrics show unavailable when there is no input-token base;
  - the Session detail page's KPI cards (peak context, cache hit rate, cost)
    now consume these helpers and never display a misleading zero or a
    complete comparison input when evidence is missing;
  - the SidechainSummary panel renders cost as unavailable when sidechain
    turns exist but token and cost evidence are all zero;
  - cost, context, diagnostics, export, and evidence panels agree on the
    same stored coverage reason; a parent/child relationship is never
    presented as merged context or aggregate cost.
- decisions:
  - source evidence remains content-free by default; an empty chart is never
    proof of zero usage, no cost, or a completed Session.
- acceptance result: users can distinguish unavailable telemetry from a
  genuine zero; no panel contradicts another panel's coverage state.
- changed files: `apps/web/app/session/[id]/session-absence.ts` (new),
  `apps/web/app/session/[id]/session-absence.test.ts` (new),
  `apps/web/app/session/[id]/page.tsx`,
  `apps/web/app/session/[id]/session-misc-panels.tsx`,
  `ARCHITECTURE.md`, `docs/diagnosis.md`, `docs/profile-model.md`,
  `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 260 passed
  - `pnpm --filter trace-server test` — 159 passed
  - `pnpm --filter agent-profile-web test` — 49 passed
  - `pnpm --filter agent-profile-web build` — passed
- known limits: the reported `019fd61d` live Session was not present in the
  test database; desktop/narrow-width visual checks are covered by the
  existing UI tests rather than a populated fixture.

<a id="t152"></a>

### T152 repository lint diagnostics cleanup

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: restore a green full-repository Biome check after the project-scope
  Task exposed ten deterministic formatting and import-order diagnostics.
- implemented scope: applied Biome's formatter and import-organization fixes to
  the ten reported Server, Web, Contracts, Core, and test files; no runtime
  logic or public contract changed.
- acceptance result: full lint is green, and the formatting-only diff contains
  no generated artifacts or unrelated refactors.
- changed files:
  `apps/server/src/__tests__/diagnosis-routes.test.ts`,
  `apps/server/src/__tests__/model-catalog.test.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts`,
  `apps/server/src/provider-config.ts`,
  `apps/server/src/routes/index.ts`, `apps/server/src/routes/provider.ts`,
  `apps/web/app/session/[id]/session-absence.test.ts`,
  `apps/web/app/settings/models/page.tsx`,
  `packages/contracts/src/home-statistics.ts`, `packages/core/src/pricing.ts`,
  and this roadmap/archive entry.
- verification:
  - `pnpm lint` — passed; Biome checked 256 files with no diagnostics
  - `pnpm test` — Core 284, Server 178, Web 60, CLI 33 tests passed
  - `pnpm build` — all workspace builds passed
  - `pnpm check:boundaries` — passed
  - `pnpm check:roadmap` — passed
  - `git diff --check` — passed
- known limits: no additional runtime issues were observed in the same
  verification pass; the separately blocked T146 npm publication remains open.

<a id="t153"></a>

### T153 semantic Provider setup path

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: make optional semantic diagnosis operable from the Web UI instead
  of exposing a run button without a visible Provider setup path.
- implemented scope:
  - added the `/settings/provider` Web page and a top-level Provider navigation
    entry; the page reads non-secret status and saves provider, Base URL, model,
    and API key through the existing Server routes;
  - kept API keys out of browser persistence and response data, clearing the
    form after save while relying on the Server's `0600` provider file;
  - linked the Session diagnosis disclosure to Provider setup and documented
    the explicit opt-in, bounded/redacted payload, deterministic fallback, and
    evidence-insufficient states;
  - corrected Provider status so legacy environment fallback is reported as
    `configSource: env` instead of being mislabeled as a file configuration;
  - added focused Web helper tests and Server coverage for the env source.
- acceptance result: users can discover, configure, and verify the Provider
  before returning to a Session and explicitly running semantic diagnosis;
  configuration alone does not send a payload.
- changed files: `apps/web/app/settings/provider/page.tsx` (new),
  `apps/web/app/provider-config.ts` (new),
  `apps/web/app/provider-config.test.ts` (new), `apps/web/app/header.tsx`,
  `apps/web/app/session/[id]/diagnosis-panels.tsx`,
  `apps/server/src/provider-config.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts`, `README.md`,
  `README.zh-CN.md`, `docs/diagnosis.md`, and the roadmap records.
- verification:
  - `pnpm lint` — passed; Biome checked 259 files with no diagnostics
  - `pnpm test` — Core 284, Server 179, Web 63, CLI 33 tests passed
  - `pnpm build` — all workspace builds passed; `/settings/provider` generated
    successfully
  - `pnpm check:boundaries` and `pnpm check:roadmap` — passed
  - `git diff --check` — passed
- known limits: the Server does not test Provider connectivity on save; the
  status page reports `untested`, and a real semantic request is still needed
  to verify the endpoint/model credentials. Sessions without captured
  token/model telemetry remain `insufficient_evidence` by design.

<a id="t154"></a>

### T154 Provider save connectivity test and protocol disclosure

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: validate Provider connectivity at configuration time and make the
  implemented wire-protocol boundary explicit.
- implemented scope:
  - saving a Provider now sends a minimal `Respond with exactly OK.` probe with
    a ten-second timeout and returns `passed` or a safe failure category;
  - added `POST /api/provider/test` and a Web “重新测试连接” action that uses
    stored Server-side credentials without asking for the API key again;
  - covered OpenAI-compatible `/chat/completions` and Anthropic-native
    `/messages` request shapes, while explicitly labeling Gemini, Azure-native,
    and Ollama-native protocols as unsupported for now;
  - probe failures expose only timeout/network/HTTP status categories, never
    response bodies, endpoint secrets, or API keys.
- acceptance result: save-time and repeat connectivity tests are visible in the
  Provider settings page; failed configurations remain editable and do not
  disable deterministic diagnosis.
- changed files: `apps/server/src/llm-diagnoser.ts`,
  `apps/server/src/runtime.ts`, `apps/server/src/routes/provider.ts`,
  `apps/server/src/__tests__/llm-diagnoser.test.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts`,
  `apps/web/app/provider-config.ts`, `apps/web/app/provider-config.test.ts`,
  `apps/web/app/settings/provider/page.tsx`, `README.md`,
  `README.zh-CN.md`, `docs/diagnosis.md`, and the roadmap records.
- verification:
  - focused Server/Web tests passed; protocol request shapes and safe failure
    categories are covered by fake fetches
  - full `pnpm lint`, `pnpm test`, `pnpm build`, `pnpm check:boundaries`,
    `pnpm check:roadmap`, and `git diff --check` passed
- known limits: the probe verifies HTTP reachability, authentication, and the
  configured model's endpoint response; it is not a quality evaluation and may
  consume a minimal provider token allowance. Native protocol adapters beyond
  the two supported wire shapes remain future work.

<a id="t155"></a>

### T155 Provider probe error diagnosis

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: make a reachable Provider rejection actionable instead of reducing
  model, authentication, endpoint, and request failures to one opaque HTTP
  status.
- implemented scope:
  - bounded non-secret probe response inspection now classifies 401/403 as
    authentication failure, 404 as endpoint-not-found, model-related 400
    responses as model-unavailable, other 400 responses as invalid-request,
    and preserves timeout/network/generic HTTP categories;
  - the Web Provider page gives corrective guidance, including that the model
    ID must be available to the configured API key and that `/models` is the
    provider-side source for model selection;
  - the Provider route tests are isolated from any real user `provider.json`,
    so local credentials cannot contaminate test expectations;
  - response bodies and API keys remain absent from the returned result, logs,
    and stored Provider status.
- acceptance result: the configured LiteLLM call was reproduced as a reachable
  request and surfaced its rejection as a typed safe failure; a later 401 from
  the same endpoint is surfaced as authentication failure, while deterministic
  diagnosis remains unaffected.
- changed files: `apps/server/src/llm-diagnoser.ts`,
  `apps/server/src/__tests__/llm-diagnoser.test.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts`,
  `apps/web/app/provider-config.ts`, `apps/web/app/provider-config.test.ts`,
  ...
  `README.zh-CN.md`, `docs/diagnosis.md`, and the roadmap records.
- verification:
  - focused Server tests — 182 passed; focused Web tests — 63 passed
  - `pnpm test` — Core 284, Server 182, Web 63, CLI 33 passed
  - `pnpm build` — all workspace builds passed; Provider settings route built
  - `pnpm lint` — Biome checked 259 files with no diagnostics
  - `pnpm check:boundaries` — passed
  - `pnpm check:roadmap` — passed
  - `git diff --check` — passed
- known limits: provider-specific error schemas remain heterogeneous; unknown
  response shapes intentionally fall back to a generic safe HTTP category.
  Native protocols beyond OpenAI-compatible and Anthropic-native remain out of
  scope, and a valid model/key pair must still be supplied by the user.

<a id="t156"></a>

### T156 Preserve saved Provider endpoint

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: stop the Provider settings page from silently replacing a saved
  custom gateway with the OpenAI default when it loads status.
- implemented scope:
  - Provider status now includes an endpoint URL with userinfo, query, and
    fragment removed; the path and protocol remain available to initialize the
    editable form;
  - the Web settings page uses that sanitized endpoint instead of blindly
    applying the protocol default, so changing a model cannot redirect a
    LiteLLM or other custom gateway to OpenAI;
  - added regression coverage for endpoint preservation and URL-credential
    removal; API key storage and non-echo behavior are unchanged;
  - documented the endpoint-preservation behavior in the Provider setup docs.
- acceptance result: the actual current OpenAI request was reproduced as a
  Provider `invalid_api_key` response, while the form-reset bug that could send
  a valid custom-gateway key to the wrong endpoint is fixed.
- changed files: `apps/server/src/provider-config.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts`,
  `apps/server/src/__tests__/diagnosis-routes.test.ts`,
  `apps/web/app/provider-config.ts`, `apps/web/app/provider-config.test.ts`,
  `apps/web/app/settings/provider/page.tsx`, `README.md`,
  `README.zh-CN.md`, `ARCHITECTURE.md`, `docs/diagnosis.md`, and the roadmap
  records.
- verification:
  - focused Server tests — 183 passed; focused Web tests — 63 passed
  - `pnpm test` — Core 284, Server 183, Web 63, CLI 33 passed
  - `pnpm build` — all workspace builds passed; Provider settings route built
  - `pnpm lint` — Biome checked 259 files with no diagnostics
  - `pnpm check:boundaries` — passed
  - `pnpm check:roadmap` — passed
  - `git diff --check` — passed
- known limits: the endpoint URL field intentionally strips query and fragment
  values; credentials must continue to be supplied through the dedicated key
  field, and protocol/model compatibility remains Provider-specific.

<a id="t157"></a>

### T157 Surface semantic diagnosis outcome

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: make the semantic Provider result visible and distinguish a parsed
  conclusion from a successful HTTP response with no usable findings.
- implemented scope:
  - semantic reports now expose the count of parsed Provider findings;
  - the Web diagnosis status distinguishes added semantic conclusions from a
    completed run with no additional conclusion, and Provider findings carry a
    visible `Provider 辅助` marker in the combined diagnosis list;
  - valid `[]` Provider output remains a completed zero-finding result, while
    missing, malformed, or schema-invalid JSON is reported as failed;
  - deterministic findings remain available and are explicitly described when
    the Provider adds no semantic finding; no raw response or prompt content is
    returned or stored.
- acceptance result: a completed semantic request now tells the user whether
  it added zero or more findings, and an unusable Provider response no longer
  masquerades as a successful empty diagnosis.
- changed files: `packages/core/src/diagnosis.ts`,
  `apps/server/src/llm-diagnoser.ts`, `apps/server/src/routes/diagnosis.ts`,
  `apps/server/src/__tests__/llm-diagnoser.test.ts`,
  `apps/server/src/__tests__/diagnosis-routes.test.ts`,
  `apps/web/app/session/[id]/diagnosis-panels.tsx`, `README.md`,
  `README.zh-CN.md`, `ARCHITECTURE.md`, `docs/diagnosis.md`, and the roadmap
  records.
- verification:
  - focused Server tests — 185 passed; focused Web tests — 63 passed
  - `pnpm test` — Core 284, Server 185, Web 63, CLI 33 passed
  - `pnpm build` — all workspace builds passed; Session route built
  - `pnpm lint` — Biome checked 259 files with no diagnostics
  - `pnpm check:boundaries` — passed
  - `pnpm check:roadmap` — passed after archiving this Task
  - `git diff --check` — passed
- known limits: semantic conclusions remain bounded, inferential, and limited
  to the three supported Provider finding types; an empty result does not prove
  the Session was optimal.

<a id="t158"></a>

### T158 Persist semantic diagnosis and disclose analysis basis

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: retain the latest bounded semantic diagnosis for a Session across
  refreshes and make the exact normalized evidence scope visible.
- implemented scope:
  - migration v21 adds one latest-result row per Session with source fingerprint,
    status/provider, bounded semantic metadata, parsed findings, and update time;
  - the repository persists only parser-redacted and length-bounded Provider
    findings, restores them through the diagnosis and analysis-summary routes,
    and fails closed on malformed stored JSON;
  - a changed Session source fingerprint invalidates the saved result, while a
    matching source restores it without another Provider call;
  - the Web disclosure states that analysis uses normalized Session Spans and
    bounded task-title/thinking/tool summaries for the three supported semantic
    patterns, and confirms that the saved result survives refresh;
  - added persistence/source-revision tests and retained deterministic diagnosis
    when no saved semantic result is available.
- acceptance result: an opt-in semantic run is now visible after page/API
  refresh without re-sending content; raw prompt, tool input, transcript,
  Provider response, and credentials remain non-persistent.
- changed files: `apps/server/src/schema.ts`,
  `apps/server/src/semantic-diagnosis-repository.ts`,
  `apps/server/src/runtime.ts`, `apps/server/src/routes/diagnosis.ts`,
  `apps/server/src/routes/sessions.ts`, `apps/server/src/routes/shared.ts`,
  `apps/server/src/__tests__/database.test.ts`,
  `apps/server/src/__tests__/semantic-diagnosis-repository.test.ts`,
  `apps/server/src/__tests__/diagnosis-routes.test.ts`,
  `apps/server/src/__tests__/session-analysis-routes.test.ts`,
  `packages/core/src/types.ts`, `packages/core/src/diagnosis.ts`,
  `apps/web/app/session/[id]/diagnosis-panels.tsx`, `README.md`,
  `README.zh-CN.md`, `ARCHITECTURE.md`, `docs/diagnosis.md`, and roadmap records.
- verification:
  - focused persistence/diagnosis tests — Server 189 passed; Web 63 passed
  - `pnpm test` — Core 284, Server 189, Web 63, CLI 33 passed
  - `pnpm build` — all workspace builds passed; Session and Provider routes built
  - `pnpm lint` — Biome checked 261 files with no diagnostics
  - `pnpm check:boundaries` — passed
  - `pnpm check:roadmap` — passed after archiving this Task
  - `git diff --check` — passed
- known limits: only the latest result per Session is retained; source changes
  invalidate it rather than attempting to reinterpret it. Semantic findings
  remain bounded, inferential, and not a delivery-quality verdict.

<a id="t151"></a>

### T151 Web development-cache ignore correction

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: correct T150's generated-cache ignore target before its first commit.
- implemented scope: replaced the repository-root-only port-cache pattern with
  `/apps/web/.next-dev-*`, which covers the Web cache directory, sidecar
  TypeScript configuration, and lock without suppressing unrelated files.
- acceptance result: `apps/web/.next-dev-3001/trace` resolves to the targeted
  `.gitignore` rule, while source `apps/web/scripts/dev.mjs` remains visible.
- changed files: `.gitignore`, `docs/roadmap.md`, and this archive entry.
- verification: `git check-ignore -v` covered both the generated cache and a
  normal source path; `pnpm check:roadmap` and `git diff --check` passed.
- known limits: the rule intentionally covers only Web-port cache names; it
  neither removes a cache nor replaces the runtime live-lock check.

<a id="t150"></a>

### T150 Isolated Next development cache recovery

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: recover the corrupted Next development output without touching
  application data, and keep ordinary local development instances from sharing
  a concurrent writer tree.
- implemented scope:
  - replaced the fixed Web development output with a validated,
    port-scoped `.next-dev-<port>` directory. Root `pnpm dev` therefore owns
    `.next-dev-3001`; the Web launcher parses `--port` and directs a separately
    started server to its own output directory;
  - added a port-specific, ignored TypeScript sidecar beside each cache. It
    extends the source configuration and references only that cache's generated
    types, so Next cannot append a competing port path to source
    `apps/web/tsconfig.json`. The sidecar deliberately lives outside the output
    tree because Next initializes and may erase its output directory at start;
  - added a same-cache PID lock. A second same-port launcher fails before it
    can write the cache. The explicit cleanup command requires `--confirm`,
    refuses a live lock, and removes only that port's cache, sidecar, and stale
    lock after the process has stopped. The launcher also rejects hostname
    overrides, retaining the existing loopback-only development boundary;
  - ignored the generated port-scoped artifacts and documented that this
    recovery is separate from the Next compiler-memory issue and from product
    data management.
- decisions:
  - no cleanup pattern can address `.next`, a database, transcripts, Sessions,
    pricing, or other application data. Cache cleanup is exact-port generated
    output only;
  - the normal launcher owns the safe default. Deliberately overriding
    `NEXT_DEV_DIST_DIR` remains possible for controlled tooling, but it is
    validated as a local `.next-dev` generated-cache name and still locked.
- acceptance result: independent temporary servers on ports 43101 and 43102
  each compiled `/settings/models` (637 modules) and returned HTTP 200 using
  separate output trees and TypeScript sidecars. A second 43101 launcher and a
  cleanup attempt while 43101 was live were rejected. After both stopped, exact
  confirmed cleanup removed only their generated artifacts. The source
  `tsconfig.json` remained unchanged, and the production build continued to
  use `.next`.
- changed files: `.gitignore`, `apps/web/package.json`,
  `apps/web/next.config.js`, `apps/web/scripts/dev-paths.mjs` (new),
  `apps/web/scripts/dev-paths.test.ts` (new), `apps/web/scripts/dev.mjs` (new),
  `apps/web/scripts/clean-dev-cache.mjs` (new), `ARCHITECTURE.md`,
  `docs/performance.md`, `docs/roadmap.md`, and this archive entry.
- verification:
  - `pnpm --filter agent-profile-web exec vitest run scripts/dev-paths.test.ts`
    — 4 passed
  - `pnpm --filter agent-profile-web test` — 18 files / 60 tests passed
  - focused Biome check for the launcher, path helpers, and Next configuration
    — passed
  - `pnpm --filter agent-profile-web build` — passed; production output stayed
    `.next`
  - production config import with an invalid `NEXT_DEV_DIST_DIR` — still used
    `.next`, so a development-only override cannot block a production build
  - two isolated localhost smokes on ports 43101 and 43102 — both compiled the
    same route and returned HTTP 200; root `tsconfig.json` had no diff
  - `dev:clean` without confirmation, against a live lock, and after stop —
    refused, refused, and then removed the exact target respectively
  - `pnpm check:roadmap` and `git diff --check` — passed
- known limits: cache isolation fixes the incomplete-output/collision class; it
  does not cap or otherwise solve the long-lived Webpack compiler footprint.
  T144's memory containment remains enabled, while Turbopack comparison and
  heap-retainer profiling are separate follow-up work.

<a id="t149"></a>

### T149 Price-revision synchronization workflow

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: make a configured exact-model price revision visibly actionable for
  pre-existing Session/Span costs without making a potentially unbounded
  historical rewrite an implicit save side effect.
- implemented scope:
  - `model-catalog/v1` inventory items now expose the durable Boolean
    `historicalCostSyncPending`. For every observed model with a supported
    current price, SQLite checks whether any stored LLM Span retains a different
    `pricing_model`, `pricing_effective_from`, or `pricing_revision`;
  - migration 20 adds `idx_spans_model_type_price_sync` for that scalar
    provenance check. The lookup returns no affected Session/Span IDs and does
    not build a matching-history array;
  - the Model Catalog pricing editor shows a nearby model-scoped notice and
    direct Preview action whenever the server-provided marker is true. This
    works after a page reload, including for a price saved before the current
    browser session;
  - successful Execute reloads inventory so the notice clears only after the
    stored provenance has actually been updated. A reload failure is reported as
    a refresh failure, not as a falsely failed recalculation;
  - Vitest now uses its supported esbuild JSX fallback for Web tests so the
    client Model Editor can be rendered and checked without changing the
    production Next configuration.
- decisions:
  - compare persisted pricing provenance instead of only a price timestamp or
    a fuzzy model label. This covers a replacement revision with the same
    effective date and explicit audited pricing aliases;
  - retain T137's Preview/Execute transaction. Saving a price reloads inventory
    only; it does not request Preview, write historical cost, or enumerate IDs.
- acceptance result: a historical Span whose unknown or old cost provenance
  differs from the currently configured exact-model price produces the durable
  marker. The user can start only that model's existing Preview from the nearby
  notice, then still must select confirmation before Execute. A successful
  matching-model execution updates the persisted price provenance and clears
  the marker; the revision check and existing update publication remain intact.
- changed files: `packages/contracts/src/model-catalog.ts`, Model Catalog
  service/schema and focused Server tests, the Model Catalog editor and Web
  test configuration/tests, `ARCHITECTURE.md`, `docs/performance.md`,
  `docs/roadmap.md`, and this archive entry.
- verification:
  - focused Model Catalog Server, route, and migration tests — 3 files / 31
    tests passed
  - focused Model Editor and catalog Web tests — 2 files / 7 tests passed
  - `pnpm --filter @agent-profile/contracts build`,
    `pnpm --filter trace-server build`, and
    `pnpm --filter agent-profile-web build` — passed
  - focused Biome check passed for 9 changed source/test files; `biome lint`
    passed for `model-catalog.test.ts`. Its full formatter check still reports
    two pre-existing formatting differences in that already-modified file, so
    they were deliberately left outside this Task
  - isolated Next development smoke — `/settings/models` returned HTTP 200
    after compiling 637 modules; private `next-server` RSS was `238,848 KB`
    (about 233 MB). The server ran on a separate loopback port with no Server
    process, source scan, or transcript database and was stopped afterward
  - `pnpm check:roadmap` and `git diff --check` — passed
- known limits: the Boolean does not disclose a count or IDs; an explicitly
  requested Preview retains its existing scope work and may be large for a
  model with a large history. The UI deliberately does not auto-run it or
  Execute. Existing historical costs are changed only after user confirmation.

<a id="t148"></a>

### T148 Model-context Session refresh linkage

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: make a saved exact-model context-window configuration promptly
  refresh open Session analysis views without rewriting imported historical
  context usage.
- implemented scope:
  - Model Catalog context writes now use a dedicated post-success callback.
    The Runtime bridges it to `SessionUpdateTracker.publish([], true)`, a
    constant-size reset event with no Session IDs;
  - configuration import stores all context rows inside its existing
    transaction and sends one reset only after a successful commit, rather than
    one event per context row;
  - existing Session-detail reset handling re-reads its bounded analysis, whose
    context-window lookup remains an exact raw-model read. Session peak/average
    context use, Span tokens, and cost fields remain historical evidence and
    are not rewritten;
  - documented the exact-identity, bounded-notification, and future
    change-level memory-safety rules.
- decisions:
  - use the existing reset contract instead of querying every Session whose
    Span has the updated model. This holds notification memory and payload size
    at O(1) in total Session count;
  - keep model-context and pricing-alias semantics separate: a pricing alias
    is not an implicit context-window alias.
- acceptance result: direct context saves and a two-row configuration import
  each advanced the cursor exactly once with `reset: true` and `sessionIds: []`.
  The current Session analysis endpoint returned the new exact-model limit
  while the selected stored Session and Span fields remained unchanged.
  The existing detail-view reset branch therefore reloads the new analysis
  without a full-history read or data rewrite.
- changed files: `apps/server/src/model-catalog/service.ts`,
  `apps/server/src/runtime.ts`, Model Catalog route/service tests,
  `ARCHITECTURE.md`, `docs/performance.md`, `docs/roadmap.md`, and this archive
  entry.
- verification:
  - Server test suite — 36 files / 169 tests passed, including the new manual
    save and two-row import bounded-reset coverage
  - `pnpm --filter trace-server build` — passed
  - `pnpm --filter agent-profile-web test` — 16 files / 54 tests passed
  - `pnpm --filter agent-profile-web build` — passed; the T144 Web compiler
    option remained enabled and reported the same 131/138 kB Home/Session
    first-load sizes
  - local `pnpm dev` smoke — the existing `next-server` was 52,224 KB RSS
    after the server-only change; no Web compiler graph/configuration changed
  - focused Biome passed for the changed Model Catalog service and route test;
    full changed-file Biome still reports pre-existing formatting in
    `model-catalog.test.ts` and pre-existing import ordering in `runtime.ts`
  - `pnpm check:roadmap` and `git diff --check` — passed
- known limits: a reset causes currently open Home, Project, and Session views
  to perform their existing bounded refetches. It does not guarantee an
  immediate browser repaint while an update request is in flight, and an
  unconfigured or differently spelled raw model still resolves to no context
  window by design.

<a id="t144"></a>

### T144 Next.js development compiler memory containment

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: reduce the peak and retained memory of the `next-server` process
  during `pnpm dev` route compilation without changing source synchronization
  or hiding pressure behind a Node heap cap.
- implemented scope:
  - enabled Next.js `experimental.webpackMemoryOptimizations` in
    `apps/web/next.config.js`, which disables dual string-buffer caching,
    interns compiler strings during Webpack phases, and purges the shared input
    filesystem after compilation;
  - replaced the shared `@lobehub/icons` root-barrel import in
    `apps/web/app/icons.tsx` with the 15 required `components/Mono` imports,
    preserving the existing icon component API while avoiding the package
    root's UI/avatar dependency graph;
  - recorded the before/after development-memory method and limitations in
    `ARCHITECTURE.md` and `docs/performance.md`.
- decisions:
  - keep Webpack as the current development bundler and use the pinned Next.js
    option as the first containment measure; do not impose
    `--max-old-space-size`, delete the user's cache, or alter Server import
    scheduling;
  - retain direct Mono imports because the intermediate icon subpath indexes
    still pulled the root barrel through `@lobehub/ui`.
- acceptance result: the same sequential nine-route smoke pass returned HTTP
  200 for every route. On Darwin arm64 / Node v24.18.0, sampled peak RSS fell
  from roughly 2.75 GB before the compiler option to 1.67 GB afterward, and
  the process settled near 115 MB after idle. The original user sample's 5.1 GB
  longer-lived high-water remains a documented limitation rather than a claim
  of a universal bound.
- changed files: `apps/web/next.config.js`, `apps/web/app/icons.tsx`,
  `ARCHITECTURE.md`, `docs/performance.md`, `docs/roadmap.md`, and this archive
  entry.
- verification:
  - `pnpm --filter agent-profile-web test` — 54 passed
  - `pnpm --filter agent-profile-web build` — passed; Home/Session first-load
    JavaScript reported 131/138 kB after the import change
  - controlled local route smoke — 9 HTTP 200 responses; sampled peak
    `1,746,480 KB`, post-idle process RSS about `115,712 KB`
  - `pnpm exec biome check apps/web/app/icons.tsx apps/web/next.config.js` —
    passed for the checked source file
  - `pnpm check:roadmap` and `git diff --check` — passed
- known limits: the measurements are local Webpack development high-water
  samples, not a cross-machine guarantee. Turbopack comparison, heap-retainer
  profiling, and longer HMR sessions remain separate follow-up work if the
  user's process again approaches multi-gigabyte RSS.

<a id="t145"></a>

### T145 Codex delegation lineage and bounded local titles

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: connect Codex Desktop delegation-created Sessions to their explicit
  parent thread and keep stored/displayed Session titles bounded to the local
  Codex title instead of persisting the full delegation envelope.
- implemented scope:
  - Codex parsing now extracts an explicit
    `<codex_delegation><source_thread_id>` parent marker from both the
    `response_item` user message and `event_msg.user_message` forms. The
    existing repository relationship path persists the recovered source-parent
    edge without inferring links from prompt wording, titles, paths, or timing;
  - Codex state metadata reads the adjacent bounded
    `session_index.jsonl` and prefers `thread_name` for local titles. State
    titles are bounded, and a delegation envelope is rejected as a title;
  - the Codex parser revision advanced to `codex-v8`, so existing derived rows
    refresh through the ordinary import/replacement path. Web Session titles
    have a 96-character display bound as a final overflow guard;
  - added focused parser, state-metadata, ingestion-revision, and Web title
    regression coverage and reconciled current architecture/multi-Agent docs.
- decisions:
  - an explicit source-thread marker is deterministic relationship evidence;
    no relationship is inferred from the embedded task text;
  - delegation wrapper input is inspected only for the parent ID and is not
    persisted as Session content or title;
  - local `session_index.jsonl.thread_name` is preferred over the state row
    title, with bounded state metadata as a safe fallback.
- acceptance result: the current child rollout shape links to its parent through
  `sourceParentSessionId` and the existing `session_relationships` write path;
  local short titles are selected and bounded; delegation envelopes do not leak
  into stored/displayed titles; ordinary title and detail navigation behavior
  remains intact.
- changed files: `packages/core/src/parsers/codex.ts` and its parser test;
  `apps/server/src/ingestion/codex-state-metadata.ts`, transcript revision,
  and ingestion/state-metadata tests; `apps/web/app/session-navigation.ts`
  and its test; `ARCHITECTURE.md`, `docs/multi-agent.md`, and roadmap records.
- verification:
  - focused Core parser test — 19 passed
  - focused Server state-metadata test — 3 passed; ingestion boundary test —
    20 passed
  - focused Web navigation test — 54 passed
  - Core, Server, and Web production builds — passed
  - `pnpm check:boundaries`, focused Biome, and `git diff --check` — passed
- known limits: an existing stored Session refreshes when the normal Codex
  `codex-v8` revision is imported; this change does not silently mutate the
  database outside the ordinary sync/rebuild workflow. The local UI's full
  lint baseline still contains unrelated pre-existing findings.

<a id="t143"></a>

### T143 Codex review initiator, no-provider evidence, and retroactive pricing

- status: completed
- started_at: 2026-08-11
- completed_at: 2026-08-11
- purpose: distinguish explicit Codex review initiator Sessions from ordinary
  primary process Sessions, keep no-context/no-Provider diagnosis states
  evidence-faithful instead of presenting missing telemetry as measured zero,
  and let configured prices cover historical Spans during explicit recalculation.
- implemented scope:
  - Codex parser metadata now records `entered_review_mode` for top-level
    rollouts; migration 19 persists the marker, direct detail labels the
    initiator, and primary discovery/statistics/cohort predicates exclude it
    without deleting the source record. Codex parser revision advanced to
    `codex-v7` so the next ordinary sync refreshes existing derived rows;
  - context windows and `/context` omit stub or `not_captured` LLM turns while
    retaining complete LLM-turn aggregates, and the Session detail explains
    unavailable context telemetry instead of rendering zero;
  - semantic diagnosis reuses the captured-context evidence predicate and the
    Web disclosure distinguishes `not_configured`, `insufficient_evidence`,
    and Provider failure, showing `Provider payload: 未发送` whenever no payload
    was sent. Deterministic diagnosis remains local without a Provider;
  - pricing lookup and Model Catalog selection now use the newest active exact
    model revision by configuration creation/row order. `effectiveFrom` remains
    provenance and no longer filters historical Spans; the existing confirmed
    preview/execute recalculation applies the selected price to all historical
    Spans in scope;
  - added focused parser, migration, discovery, context, semantic-status, and
    retroactive-recalculation regression coverage and reconciled current-state
    README, architecture, pricing, statistics, diagnosis, and multi-Agent docs.
- decisions:
  - `event_msg.payload.type=entered_review_mode` is the only review marker;
    titles, prompts, paths, and timing never infer the role;
  - saving a price revision remains non-mutating. Historical stored costs are
    changed only by the existing fixed-revision preview/execute flow (or the
    compatibility full recompute endpoint), which now includes old Spans;
  - `effectiveFrom` is retained for audit/history but is not a billing-time
    gate under the approved retroactive correction policy.
- acceptance result: review initiators are labeled and retained by direct ID
  while excluded from ordinary primary aggregates; missing context/provider
  evidence is unavailable/not-sent rather than zero or a false Provider error;
  deterministic diagnosis works without an LLM key; and a newly configured
  four-token price recalculates historical Spans regardless of Span time.
- changed files: Codex/core types, parser, analyzer, session-analysis and tests;
  Server schema, pricing lookup/catalog, repository, primary scope, routes,
  transcript revision, and tests; Session-detail Web/UI helpers and model-editor
  copy; `README.md`, `ARCHITECTURE.md`, `docs/model-configuration-design.md`,
  `docs/stats.md`, `docs/diagnosis.md`, `docs/multi-agent.md`, and roadmap records.
- verification:
  - `pnpm --filter @agent-profile/core test` — 281 passed
  - `pnpm --filter trace-server test` — 168 passed
  - `pnpm --filter agent-profile-web test` — 54 passed
  - `pnpm --filter @agent-profile/contracts build`,
    `pnpm --filter @agent-profile/core build`,
    `pnpm --filter trace-server build`, and
    `pnpm --filter agent-profile-web build` — passed
  - `pnpm check:boundaries`, `pnpm check:roadmap`, focused Biome over 19
    changed code files, and `git diff --check` — passed
  - full `pnpm lint` remains blocked by 13 pre-existing formatting/import
    findings in unrelated files; no new findings remain in the focused check.
- known limits: existing stored Sessions receive the review marker on the next
  Codex sync/rebuild because the parser fingerprint is versioned; saving a new
  price still requires an explicit recalculation confirmation to update stored
  costs. No Provider key is created automatically, so semantic diagnosis stays
  deterministic-only until the user configures one.
...
- started_at: 2026-08-10
- completed_at: 2026-08-10
- purpose: prevent copied parent-context turn IDs from overwriting parent or
  sibling Codex Spans, and make evidence/API/UI absence and ownership states
  source-faithful.
- implemented scope:
  - Codex normalization now merges adjacent buckets for one raw turn, scopes
    every generated ID and same-Session parent ID as
    `codex:<session>:<source-id>`, and retains copied parent context as
    `ownershipStatus=cross_session` without inheriting its model or token usage;
  - parser revision advanced to `codex-v6`, and migration 18 backfills the
    same-session scope for existing Codex rows through a temporary prefix while
    preserving annotations and unresolved references;
  - core and Server evidence DTOs preserve `cross_session`, `source_user`,
    `corrupted_ownership`, and `not_captured` parent/reference states, and
    non-LLM events no longer expose model or cost values as applicable metrics;
  - Session-detail Web evidence renders LLM token/model/cost fields only for
    LLM turns and labels tool/block-specific evidence separately;
  - focused parser, migration, Server route, core evidence, and Web display
    regression tests cover deterministic child/parent imports and block-level
    absence semantics.
- decisions:
  - the existing global Span primary key remains compatible; deterministic
    Session-scoped Codex IDs avoid a broad composite-key migration;
  - inherited parent context remains inspectable for auditability but is never
    counted as child LLM usage, and missing ownership fields remain uncaptured;
  - ownership markers are additive metadata and evidence status, not inferred
    delivery quality or causal comparison signals.
- acceptance result: parent and child Codex rollout imports are deterministic
  and Session-scoped; evidence distinguishes linked, cross-Session, source-user,
  corrupted-ownership, and not-captured states; thinking/answer blocks no
  longer render inherited LLM fields as zero or missing.
- changed files: Codex parser, core analyzer/diagnosis/session-analysis/types and
  evidence modules plus focused tests; Server transcript adapter, route,
  schema, evidence service, LLM diagnoser, and migration/ingestion/evidence
  tests; CLI contracts; Session-detail Web page/panel/evidence helpers/tests;
  `ARCHITECTURE.md`, `docs/multi-agent.md`, and roadmap records.
- verification:
  - `pnpm test` — Core 280, Server 166, Web 54, CLI 30 passed
  - `pnpm --filter @agent-profile/core build`,
    `pnpm --filter @agent-profile/contracts build`,
    `pnpm --filter trace-server build`, and
    `pnpm --filter agent-profile-web build` — passed
  - `pnpm check:boundaries`, `pnpm check:roadmap`, focused Biome, and
    `git diff --check` — passed
  - local smoke over 50 Codex rollout files — 5,860 parsed Spans, zero
    duplicate IDs after Session scoping and turn merging
  - `pnpm build` reached successful Core, Contracts, Server, and Web builds but
    stopped at the pre-existing CLI `ModelIdentityKind`/`CliStatsData` mismatch
    outside this Task's scope
  - additional regression coverage verifies copied parent turns without an
    explicit child boundary, excludes inherited blocks from child aggregates
    and parent-link coverage, and keeps the browser evidence helper free of
    server-only core imports
  - `codex review --uncommitted` was rerun as the single review route after
    fixing its P1 nullable-model build finding; state initialization remained
    read-only and the escalated local provider returned HTTP 426, so the
    configured manual checklist completed with no blocking findings (the first
    manual pass also reported and was fixed for the P2 context-query regression)
- known limits: source-user/corrupted/not-captured statuses are preserved when
  the source or normalized metadata marks them; the parser does not invent a
  user-message Span or reconstruct missing cross-Session source content.

<a id="t141"></a>

### T141 Codex metadata and agent-lineage evidence repair

- status: completed
- started_at: 2026-08-10
- completed_at: 2026-08-10
- purpose: align imported Codex Session identity with the source-of-truth
  state database and preserve deterministic child-agent lineage evidence for
  Session detail navigation and review.
- implemented scope:
  - added a read-only, schema-adaptive state metadata index for the newest
    `~/.codex/state_*.sqlite`, matching rollout paths or thread IDs and
    fingerprinting title, agent identity, parent edges, and child metadata;
  - Codex imports now use the exact state-record title when available, keep a
    missing title absent instead of deriving it from reasoning, and carry
    source nickname/role/path plus parent/child thread IDs through normalization;
  - captured `sub_agent_activity` start/interacted events and source agent
    messages provide call/callback timestamps and `observed` versus
    `final_answer` status without persisting callback text;
  - migration 17 adds nullable relationship evidence columns; repository
    upserts preserve evidence contributed by either import order and metadata
    changes prevent an append path from skipping a required full replacement;
  - relationship API responses omit unavailable fields, and Session detail
    shows deterministic child identity plus captured call/callback evidence,
    including when the counterpart Session is unavailable.
- decisions:
  - only deterministic rollout/state fields are accepted; titles, paths,
    timestamps, models, and callback text are not used to infer relationships
    or stored as prompt/process content;
  - absent source fields remain omitted/unknown, and optional state database
    failures leave transcript import available without fabricating metadata;
  - parent and child imports merge relationship evidence by source identity so
    importing one side cannot erase fields captured by the other side.
- acceptance result: exact Codex titles, named-agent identity, source call and
  callback evidence, and observed/final-answer status now survive import,
  storage, API normalization, and Web display; missing fields remain absent;
  no callback text is persisted.
- changed files: `packages/core/src/index.ts`,
  `packages/core/src/types.ts`, `packages/core/src/parsers/codex.ts`,
  `packages/core/src/__tests__/codex-parser.test.ts`,
  `apps/server/src/ingestion/codex-state-metadata.ts`,
  `apps/server/src/ingestion/transcript-adapter.ts`,
  `apps/server/src/ingestion/session-repository.ts`,
  `apps/server/src/ingestion/types.ts`, `apps/server/src/schema.ts`,
  `apps/server/src/session-relationships.ts`, focused Server tests,
  `apps/web/app/session/[id]/source-relationship.ts`,
  `apps/web/app/session/[id]/source-relationship-card.tsx`, its focused test,
  `ARCHITECTURE.md`, `docs/multi-agent.md`, and the roadmap register
- verification:
  - `pnpm test` — Core 272, Server 164, Web 51, CLI 30 passed
  - `pnpm --filter @agent-profile/core build` — passed
  - `pnpm --filter trace-server build` — passed
  - `pnpm --filter agent-profile-web build` — passed
  - `pnpm check:boundaries` and `pnpm check:roadmap` — passed
  - focused Biome checks for all changed TypeScript/TSX files and
    `git diff --check` — passed
  - read-only smoke against the local Codex state DB matched the current
    parent rollout title and nine child lineage records, including captured
    call/callback times and final-answer status
  - `codex review --uncommitted` could not initialize in the sandbox and the
    escalated retry reached the local provider but failed with HTTP 426;
    the configured manual review checklist was completed as the documented
    fallback, with no blocking findings
  - `pnpm build` reached successful Core, Contracts, Server, and Web builds but
    stopped at the pre-existing CLI `ModelIdentityKind`/`CliStatsData` type
    mismatch outside this Task's scope.
- known limits: state metadata remains optional and schema-adaptive; fields
  absent from the rollout/state records are intentionally not reconstructed.
  T142 remains planned for Session-scoped Span ownership and block-level
  evidence status repair; the unrelated CLI build mismatch remains open.
...
- implemented scope:
  - added a server-only Provider configuration store (`provider.json` in the
    application data directory, mode `0600`); the API key never reaches
    browser state, localStorage, SQLite, logs, exports, or source files, and
    environment variables remain a legacy fallback only when no file exists;
  - added `GET /api/provider/status` (non-secret: provider, model, endpoint
    host, loopback/external locality, configuration source, test status,
    restart requirements, key presence) and
    `PUT /api/provider/configuration` to store an explicit provider, base
    URL, model, and key; responses never echo the key;
  - the diagnosis route now resolves the Provider through the runtime
    provider service instead of a module-level environment default, retains
    request-scoped `semantic=opt_in` consent, and reports
    `not_configured` when no Provider is configured;
  - semantic conclusions are suppressed when no LLM turn in the Session has
    captured token/model telemetry (`tokenUsageSource=not_captured` or stub
    turns): the diagnosis reports `insufficient_evidence` and never calls the
    Provider;
  - `SemanticDiagnosisStatus` and the CLI report contract gained
    `insufficient_evidence`.
- decisions:
  - LLM inference cannot repair missing telemetry or prove causality; no
    Provider call occurs merely because a key is present;
  - every Provider call remains explicitly consented, bounded, redacted, and
    auditable without raw payload retention.
- acceptance result: an unconfigured user receives a clear local setup path
  before clicking Run; no secret reaches the browser or repository; every
  Provider call remains explicitly consented, bounded, redacted, and auditable
  without raw payload retention; evidence-insufficient Sessions report
  `insufficient_evidence` instead of guessing.
- changed files: `apps/server/src/provider-config.ts` (new),
  `apps/server/src/routes/provider.ts` (new),
  `apps/server/src/runtime.ts`, `apps/server/src/routes/diagnosis.ts`,
  `apps/server/src/routes/index.ts`, `packages/core/src/diagnosis.ts`,
  `packages/contracts/src/cli.ts`,
  `apps/server/src/__tests__/provider-routes.test.ts` (new),
  `apps/server/src/__tests__/diagnosis-routes.test.ts`, `README.md`,
  `ARCHITECTURE.md`, `docs/diagnosis.md`, `docs/profile-model.md`,
  `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 260 passed
  - `pnpm --filter trace-server test` — 158 passed
  - `pnpm --filter agent-profile-web test` — 45 passed
  - `pnpm --filter @agent-profile/contracts build` — passed
  - `pnpm --filter @agent-profile/core build` — passed
  - `pnpm --filter trace-server build` — passed
- known limits: a local UI for Provider configuration is not yet rendered;
  the API contract is covered by route tests and the README documents the
  setup path. Provider test-status probing is exposed as a field but not yet
  wired to a live connectivity check.
...
- implemented scope:
  - Preview remains read-only and binds Execute to the reviewed pricing
    revision and optional model/time scope; the existing transaction
    recomputes every affected Session from all of its LLM turns while
    preserving unknown/excluded coverage, and records a bounded
    recalculation audit (before/after coverage, affected scope) without
    storing source content;
  - after a successful Execute, the runtime publishes a content-free
    session-update signal listing the affected Session IDs so open discovery,
    statistics, Profile, and detail surfaces refetch instead of showing stale
    aggregates; Preview publishes no signal;
  - full-dataset recalculation remains a separately confirmed action and is
    never an implicit side effect of saving a price revision.
- decisions:
  - recalculation never changes tokens, context evidence, source model
    strings, or Session relationships; scoped runs do not reset unrelated
    Sessions, and a failed run leaves all aggregates unchanged.
- acceptance result: Preview and Execute report the same deterministic scope;
  visible aggregates refresh after a successful run; a Session with mixed
  known and unknown turns retains both its known subtotal and
  incomplete-coverage status.
- changed files: `apps/server/src/model-catalog/service.ts`,
  `apps/server/src/runtime.ts`,
  `apps/server/src/__tests__/model-catalog.test.ts`, `ARCHITECTURE.md`,
  `docs/stats.md`, `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 260 passed
  - `pnpm --filter trace-server test` — 155 passed
  - `pnpm --filter agent-profile-web test` — 45 passed
  - `pnpm --filter trace-server build` — passed
- known limits: Web refresh behavior relies on the existing
  `/api/session-updates` long-poll loop already used by the home page; other
  open surfaces (stats/Profile) refetch on their own navigation or polling
  lifecycle. Scale regression for bounded result payloads remains covered by
...
- purpose: ensure cost is only represented as known when source token coverage
  ...
  time.
- implemented scope:
  - introduced a persisted, API-visible `costStatus` on every LLM Span:
    `known`, `unknown_pricing`, `unverified_provider_route`,
    `token_usage_not_captured`, `unsupported_scheme`, `excluded_synthetic`,
    and `not_applicable` for non-LLM Spans;
  - Sessions derive `complete` / `partial` / `unknown` / `excluded` /
    `not_captured` from their LLM turns in both the analyzer and the
    recalculation path;
  - `calcCost` marks synthetic placeholders as `excluded_synthetic`, missing
    token usage as `token_usage_not_captured`, review-required provider
    routes as `unverified_provider_route`, missing pricing as
    `unknown_pricing`, and unsupported schemes as `unsupported_scheme`; only
    `known` is a trusted bill;
  - migration 15 (`evidence_safe_cost_status`) adds `spans.cost_status` and
    `sessions.cost_status`; migration 16 (`retire_synthetic_zero_price_seed`)
    retires the active zero-price `<synthetic>` bundled seed, and the seed no
    longer re-inserts it; configuration import skips excluded/superseded rows;
  - stats surfaces now expose `sessionsWithKnownCost` and `sessionsExcluded`
    alongside `sessionsWithCostUnknown`, and the Web stats page labels known /
    unknown / excluded Sessions separately instead of implying unknown is
    free;
  - evidence events expose `metrics.costStatus` so detail surfaces can render
    coverage instead of a misleading zero.
- decisions:
  - a price of zero is valid only with an explicit, time-bounded provider
    tariff; subscription fees and unknown routing are never invented from
    token records;
...
- acceptance result: unknown or incomplete evidence cannot render as `¥0`, a
  zero cost, or a complete comparison input; all four token classes remain
  separate; historical pricing provenance remains inspectable; synthetic
  placeholder data is excluded from billing entirely.
- changed files: `packages/core/src/types.ts`,
  `packages/core/src/pricing.ts`, `packages/core/src/analyzer.ts`,
  `packages/core/src/session-evidence.ts`,
  `packages/contracts/src/home-statistics.ts`,
  `apps/server/src/schema.ts`, `apps/server/src/routes/shared.ts`,
  `apps/server/src/ingestion/session-repository.ts`,
  `apps/server/src/model-catalog/defaults.ts`,
  `apps/server/src/model-catalog/service.ts`,
  `apps/server/src/routes/stats.ts`,
  `apps/web/app/stats-charts.tsx`, `apps/web/app/stats/page.tsx`,
  `apps/server/src/__tests__/database.test.ts`,
  `apps/server/src/__tests__/model-catalog.test.ts`, `README.md`,
  `ARCHITECTURE.md`, `docs/stats.md`, `docs/roadmap.md`,
  `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 260 passed
  - `pnpm --filter trace-server test` — 154 passed
  - `pnpm --filter agent-profile-web test` — 45 passed
  - `pnpm --filter @agent-profile/contracts build` — passed
  - `pnpm --filter @agent-profile/core build` — passed
  - `pnpm --filter trace-server build` — passed
  - `pnpm --filter agent-profile-web build` — passed
- known limits: historical cost recalculation UI polish and dependent-data
  refresh signals remain bounded by T137; Session-detail absence rendering for
  cost remains bounded by T140.
...
- implemented scope:
  - extended the core identity contract with a billing-eligibility
    classification: `billable`, `review_required`, and `excluded` on every
    recognized model identity;
  - classified `<synthetic>` as a non-billable synthetic placeholder,
    `astron-code-latest` as an opaque rolling source label, and `big-pickle`
    as a provider-managed route pending tariff evidence; all raw values remain
    inspectable in inventory and statistics;
  - the Model Catalog inventory now exposes `identityKind` and
    `billingEligibility` per observed raw label, and pricing/context
    configuration is rejected for runtime, synthetic, opaque, and
    review-required labels unless an explicit audited pricing alias points at
    a configurable model;
  - configuration import skips excluded labels (legacy synthetic rows) instead
    of failing on them;
  - the settings page groups the model list into billable / review /
    excluded identity buckets with labels, and explicit aliases remain the
    audited path for provider-managed route labels.
- decisions:
  - a canonical display identity, a pricing alias, a context-window
    equivalence, and a billing identity are separate claims;
  - no automatic name-similarity alias may turn a rolling/opaque label into a
    concrete model; only explicit audited aliases can enable pricing.
- acceptance result: the catalog visibly separates billable, review-required,
  and excluded source labels; only explicit auditable evidence can enable
  pricing or context configuration; statistics retain raw-label inspectability.
- changed files: `packages/core/src/model-identity.ts`,
  `packages/contracts/src/model-catalog.ts`,
  `apps/server/src/model-catalog/service.ts`,
  `apps/web/app/settings/models/model-catalog.ts`,
  `apps/web/app/settings/models/page.tsx`,
  `apps/web/app/settings/models/model-catalog.module.css`,
  `packages/core/src/__tests__/model-identity.test.ts`,
  `apps/server/src/__tests__/model-catalog-routes.test.ts`,
  `apps/web/app/settings/models/model-catalog.test.ts`, `ARCHITECTURE.md`,
  `docs/stats.md`, `docs/profile-model.md`, `docs/roadmap.md`,
  `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 253 passed
  - `pnpm --filter trace-server test` — 152 passed
  - `pnpm --filter agent-profile-web test` — 45 passed
  - `pnpm --filter @agent-profile/contracts build` — passed
  - `pnpm --filter @agent-profile/core build` — passed
  - `pnpm --filter trace-server build` — passed
  - `pnpm --filter agent-profile-web build` — passed
- known limits: the active zero-price `<synthetic>` bundled seed still exists
  in the pricing table; retiring it safely is bounded by T136 (additive
  migration/backfill that never treats missing data as free usage).

<a id="t134"></a>

### T134 source telemetry coverage and Session-relationship integrity

- status: completed
- started_at: 2026-08-10
- completed_at: 2026-08-10
- purpose: make missing source telemetry, invalid imported records, and Codex
  parent/child evidence explicit so a Session such as `019fd61d` never appears
  to have measured zero context or a guessed relationship.
- implemented scope:
  - defined a source-faithful token-usage origin contract on LLM Spans:
    `message_usage`, `token_count`, `total_tokens_fallback`,
    `session_aggregate`, `request_token_usage`, or `not_captured`, with
    classified and stub flags; the origin is stored in Span metadata and
    never inferred from names, paths, timestamps, or content;
  - Codex parser now retains a turn whose bucket contains only a
    `turn_context` (with a model identity) as a stub LLM Span with
    `tokenUsageSource=not_captured`, `stubTurn=true` and zero tokens, instead
    of dropping the started-but-untracked turn; classified `token_count`
    events are tagged `token_count`/classified, and
    `total_tokens_fallback` remains explicitly unclassified;
  - Claude, MiMo, Zed, and OpenCode parsers now tag every LLM Span with its
    captured origin (`message_usage`, `request_token_usage`,
    `session_aggregate`) or `not_captured` when the source has no usage;
  - the session-evidence report and evidence-page contract now expose
    per-event token-usage coverage (`captured`/`not_captured`/
    `not_applicable`, source, classified flag, stub flag) and a Session-level
    `tokenUsage` coverage row alongside model identity and content coverage;
  - added fixtures for an ordinary Codex turn, a stub turn, a fallback
    token-count turn, and a child Session whose source parent is unavailable,
    asserting the child never merges parent tokens or cost.
- decisions:
  - missing source fields remain `not_captured`, never a numerical zero;
  - a parent relationship is navigation/evidence only; it never merges the
    parent's tokens or context into a child Session;
  - raw source evidence is retained and inspected through the ordinary
    coordinator/repository path without rewriting source transcripts.
- acceptance result: affected Sessions now have a source-faithful reason for
  absent context/model/token data; normal Codex turns retain their own LLM
  evidence; no source data is deleted or manually rewritten.
- changed files: `packages/core/src/types.ts`,
  `packages/core/src/session-evidence.ts`,
  `packages/core/src/parsers/codex.ts`, `packages/core/src/parsers/claude.ts`,
  `packages/core/src/parsers/mimo.ts`, `packages/core/src/parsers/opencode.ts`,
  `packages/core/src/parsers/zed.ts`,
  `apps/server/src/session-evidence-service.ts`,
  `packages/core/src/__tests__/codex-parser.test.ts`,
  `packages/core/src/__tests__/session-evidence.test.ts`,
  `packages/core/src/__tests__/opencode-parser.test.ts`,
  `apps/server/src/__tests__/session-evidence-routes.test.ts`,
  `apps/server/src/__tests__/ingestion.test.ts`, `ARCHITECTURE.md`,
  `docs/diagnosis.md`, `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `pnpm --filter @agent-profile/core test` — 252 passed
  - `pnpm --filter trace-server test` — 150 passed
  - `pnpm --filter agent-profile-web test` — 44 passed
  - `pnpm --filter @agent-profile/contracts build` — passed
  - `pnpm --filter @agent-profile/core build` — passed
  - `pnpm --filter trace-server build` — passed
- known limits: the reported live Session `019fd61d` was not present in the
  checked-in `trace.db` (empty database), so the read-only inspection and
  per-Session backfill assertion is covered by ingestion/evidence fixtures
  rather than that exact record; pricing eligibility and Session-detail
  presentation remain bounded by T135/T136/T140.

<a id="t133"></a>

### T133 evidence-safe remediation task decomposition

- status: completed
- started_at: 2026-08-10
- completed_at: 2026-08-10
- purpose: turn the reported Session telemetry, model identity, cost,
  semantic-Provider configuration, and discovery-facet defects into bounded
  work with evidence-safe acceptance criteria.
- implemented scope: added seven sequenced planned Tasks (T134–T140) to the
  current register. The plan separates source telemetry and relationship
  integrity; model/billing eligibility; cost-status/schedule correctness;
  historical recalculation and refresh; server-only semantic Provider setup;
  Project × Agent facets; and Session-detail absence states.
- decisions:
  - missing, opaque, provider-only, runtime, synthetic, and unpriced evidence
    must never render as a numerical zero, a concrete model, or complete
    comparison coverage;
  - raw source evidence remains preserved and inspectable; no planned Task may
    delete transcripts, Span model strings, or relationships to simplify UI;
  - CNY-only pricing remains current, and this programme does not revive the
    cancelled multi-currency/remote-governance scope;
  - LLM-assisted analysis requires server-only configuration, explicit consent,
    bounded/redacted payloads, and evidence-insufficiency disclosure.
- acceptance result: each reported defect has one owner, explicit dependencies,
  source/data safety constraints, acceptance criteria, verification plan, and
  documentation obligation; high-risk Server and Web work is sequenced.
- changed files: `docs/roadmap.md`, `docs/roadmap-archive/2026-q3.md`
- verification:
  - `git diff --check` — passed
  - `pnpm check:roadmap` — passed
- known limits: this planning Task does not inspect or mutate the user's live
  application database, configure a Provider key, change source parsing, or
  alter historical prices; those actions remain bounded by T134–T140.
...
- started_at: 2026-08-11
- completed_at: 2026-08-11
- execution_note: T146 remains blocked on maintainer npm authentication and was
  intentionally deferred. This Task was implemented against the existing
  source-workspace CLI surface; npm package identity and publication remain
  outside its scope.
- purpose: let a CLI user explicitly select one local project without treating
  every user-level Agent history record as belonging to that project.
- implemented scope:
  - added `--project <path>` validation and root normalization to Runtime CLI
    commands. Explicit `--database`/`--data-dir` still win; otherwise project
    data defaults to `<project>/.agent-profile/trace.db`, which is ignored by
    Git;
  - threaded the project root through Runtime, import adapters, stored-count
    status, Session discovery, Home/Stats/Profile aggregates, and Web scope
    labels;
  - added one path-boundary classifier. Captured cwd inside the root is
    included, outside is excluded, and missing cwd is unassigned. Import
    results preserve bounded discovered/included/excluded/unassigned coverage;
  - made generated-data reset transactional and project-scoped across Sessions,
    Spans, and source relationships while retaining Tasks, Outcomes,
    configuration snapshots, cohorts, experiments, and logical links;
  - added scoped data-management summaries and CLI/Web reporting so current
    project and global data are visibly distinct.
- decisions:
  - analytical `project_key` remains a grouping/filter and is not reused as a
    destructive data boundary;
  - process cwd is never silently treated as a project selection;
  - missing source cwd remains unassigned rather than guessed into a project;
  - a project-scoped rebuild leaves rows from other roots untouched, and an
    explicit database override remains allowed but still scopes imports and
    destructive operations by the selected root.
- acceptance result: a user can select an existing project explicitly; imports
  report included/excluded/unassigned coverage; session discovery and aggregate
  views use the same scope; the UI distinguishes current-project/global scope;
  scoped rebuild/reset cannot delete another project's generated rows; and
  project-local data is ignored by version control.
- changed files: `.gitignore`, `README.md`, `README.zh-CN.md`, `ARCHITECTURE.md`,
  `docs/multi-agent.md`, `docs/roadmap.md`, this archive, and
  `apps/server/package.json`; `apps/server/src/data-path.ts`,
  `apps/server/src/project-scope.ts`, `apps/server/src/runtime.ts`,
  `apps/server/src/ingestion/{import-coordinator,import-runtime,import-service,session-repository}.ts`,
  `apps/server/src/session-discovery-service.ts`,
  `apps/server/src/reports-service.ts`,
  `apps/server/src/routes/{profiles,scan,sessions,stats}.ts`, and the three
  project-scope/data-management tests; `packages/cli/src/{arguments,main,runner,serve}.ts`
  and `runner.test.ts`; `apps/web/app/data-management-dialog.tsx`,
  `apps/web/app/import-state.ts`, `apps/web/app/page.tsx`; and
  `packages/contracts/src/cli.ts`, `packages/contracts/src/common.ts`,
  `packages/contracts/src/index.ts`, plus `packages/core/src/types.ts`.
- verification:
  - `pnpm test` — Core 284, Server 178, Web 60, CLI 33 tests passed;
  - `pnpm build` — Core, Contracts, Server, Web, and CLI builds passed;
  - `pnpm check:boundaries` — passed;
  - `pnpm check:roadmap` — passed;
  - focused Biome check on all changed source/test files — passed;
  - `git diff --check` — passed;
  - clean-project smoke: `node packages/cli/bin/agent-profile.mjs doctor
    --project <temporary-directory> --json` created
    `<project>/.agent-profile/trace.db` and returned `scope.mode=project`;
  - full Biome check remains blocked by 11 pre-existing formatting/import
    diagnostics outside this Task; no unrelated baseline files were changed.
- known limits: npm package distribution remains deferred to T146, and source
  records with no cwd remain visible only through the unassigned coverage state
  in a project-scoped database; no source adapter can infer a missing cwd. An
  out-of-scope cwd change removes only an unannotated stale row that was already
  inside the selected project; annotated rows remain protected for manual action.
