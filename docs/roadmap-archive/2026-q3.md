...
- acceptance result: users can discover and configure the Provider before
  returning to a Session and explicitly running semantic diagnosis;
...
- verification:
  - focused Server and Web Provider tests passed
  - Server TypeScript build passed; Web production build passed
  - focused Biome check, `pnpm check:roadmap`, and `git diff --check` passed
