...
- 从 Home 选择 Session 后，工具栏必须立刻保留已选 Agent 与标题，并在详情内容区展示
  固定布局的加载状态；不得把 iframe 首次 `load` 当作分析数据就绪，也不得留下空白或
  未变化的 Dashboard。
- 嵌入详情只可用同源、当前 iframe、当前 Session ID 的 `ready`/`error` 状态结束过渡；
  快速切换后的旧消息和其他窗口消息必须忽略。错误状态须说明可返回总览重新选择，而不是
  显示旧会话内容。该反馈不移动焦点、不使用计时承诺或页面级 loading 动画。
