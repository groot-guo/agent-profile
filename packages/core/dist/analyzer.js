...
export function analyzeSession(parsed, pricingLookup, fileMeta, importedAt) {
    let inputTokens = 0;
...
export function analyzeEfficiency(spans) {
    ...
    const toolSuccessRates = [...toolStats.entries()]
    ...
    const thinkingActionRatios = turns.map((turn, i) => {
        // Find thinking spans that are children of this turn
        const childThinkings = thinkings.filter((th) => th.parentId === turn.id);
        const thinkingChars = childThinkings.reduce((sum, th) => sum + (typeof th.metadata?.thinking === 'string' ? th.metadata.thinking.length : 0), 0);
    ...
    const contextGrowthVelocity = turns.length > 1
        ? Math.round(contextGrowthPerTurn.slice(1).reduce((s, t) => s + Math.max(0, t.delta), 0) / (turns.length - 1))
        : 0;
    // 4. File operations
    const fileOps = new Map();
    for (const t of tools) {
        if (!FILE_TOOLS.has(t.name))
            continue;
...
export function calcEfficiencyScore(efficiency, cacheHitRate, totalTokens, outputTokens, totalCost, wastedCost) {
    ...
    const wasteAvoidance = totalCost > 0 && wastedCost != null
        ...
        : 1;
    const score = Math.round((tokenEfficiency * 0.2 + cacheUtilization * 0.3 + toolSuccess * 0.2 + wasteAvoidance * 0.3) * 100);
    return {
...
export function analyzeCostAttribution(spans, wastedCost) {
    ...
    for (const [cat, turnSet] of catTurns) {
        const entry = catCost.get(cat);
        if (entry)
            entry.turnCount = turnSet.size;
    }
    const costByCategory = [...catCost.entries()]
        .map(([category, e]) => ({
...
const p95 = (arr) => { const s = [...arr].sort((a, b) => a - b); return s[Math.max(0, Math.floor(s.length * 0.95))] || 0; };
const median = (arr) => { const s = [...arr].sort((a, b) => a - b); return s[Math.max(0, Math.floor(s.length * 0.5))] || 0; };
...
export function analyzePerformance(spans) {
    const turns = spans.filter((s) => s.type === 'llm_turn').sort((a, b) => a.startTime - b.startTime);
    const tools = spans.filter((s) => s.type === 'tool_call');
    // Turn latency
...
export function analyzeToolParams(spans) {
    const tools = spans.filter((s) => s.type === 'tool_call').sort((a, b) => a.startTime - b.startTime);
    // Bash command classification
    const bashCats = new Map();
    for (const t of tools) {
        if (t.name !== 'Bash')
            continue;
