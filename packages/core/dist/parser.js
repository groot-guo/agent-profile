...
function asBlocks(msg) {
    if (!msg || !Array.isArray(msg.content))
        return [];
    return msg.content.filter((b) => typeof b === 'object' && b !== null && 'type' in b);
}
function asToolResults(msg) {
    if (!msg || !Array.isArray(msg.content))
        return [];
    return msg.content.filter((b) => typeof b === 'object' && b !== null && b.type === 'tool_result');
}
function makeSpan(p) {
    return {
        id: p.id,
...
export function parseTranscript(entries, opts) {
    if (entries.length === 0)
        return null;
        ...
        for (const b of asBlocks(e.message)) {
            if (b.type === 'thinking') {
                spans.push(makeSpan({
                    ...
                    isSidechain: e.isSidechain,
                    metadata: {
                        thinking: truncate(typeof b.thinking === 'string' ? b.thinking : safeStringify(b.thinking)),
                    },
                }));
            }
...
function findNextTimestamp(sorted, fromIdx) {
    for (let j = fromIdx; j < sorted.length; j++) {
        if (sorted[j].timestamp)
            return toMs(sorted[j].timestamp);
    }
    return undefined;
}
function extractAiTitle(entries) {
