...
import {
  findTranscriptFiles, readTranscript, parseTranscript, analyzeSession, diagnoseSession,
  type SessionSummary, SessionDetail, Span, Pricing, ScanResult,
} from '@agent-profile/core';
import { db, getPricing, getModelContext } from './db';

interface ScanBody { dir: string; }
interface PricingBody {
  model: string; inputPrice: number; cacheCreationPrice: number;
  cacheReadPrice: number; outputPrice: number; effectiveFrom?: number;
}
interface ModelContextBody { model: string; contextWindow: number; }
...
function parseSpanRow(s: Record<string, unknown>): Span {
  ...
  if (s.metadata && typeof s.metadata === 'string') {
    try { s.metadata = JSON.parse(s.metadata as string); } catch { /* keep */ }
  }
...
export function registerRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: ScanBody }>('/api/scan', async (req, reply) => {
    ...
    const tx = db.transaction((rows: Span[]) => {
      for (const s of rows) insertSpan.run({
    ...
    for (const file of files) {
      ...
      const existing = getExisting.get(parsed.sessionId) as { file_mtime: number; file_size: number } | undefined;
      if (existing && existing.file_mtime === mtime && existing.file_size === size) {
        skipped++; continue; // 未变
      }
      if (existing) { delSpans.run(parsed.sessionId); delSession.run(parsed.sessionId); updated++; }
      else { imported++; }
      ...
      insertSession.run({
        ...
        cwd: summary.cwd ?? null, gitBranch: summary.gitBranch ?? null,
        ...
        cacheHitRate: summary.cacheHitRate, messageCount: summary.messageCount,
  ...
  app.get('/api/sessions', async () => {
    return db.prepare(`SELECT ${SESSION_COLS} FROM sessions ORDER BY start_time DESC`).all() as SessionSummary[];
  });

  app.get<{ Params: { id: string } }>('/api/session/:id', async (req, reply) => {
    const session = db.prepare(`SELECT ${SESSION_COLS} FROM sessions WHERE id = ?`).get(req.params.id) as SessionSummary | undefined;
    if (!session) return reply.status(404).send({ error: 'session not found' });
    const spans = db.prepare(`SELECT ${SPAN_COLS} FROM spans WHERE session_id = ? ORDER BY start_time ASC`).all(req.params.id) as Record<string, unknown>[];
    return { ...session, spans: spans.map(parseSpanRow) } as SessionDetail;
  });

  // 每轮 LLM 调用明细
  app.get<{ Params: { id: string } }>('/api/session/:id/turns', async (req) => {
    const rows = db.prepare(`SELECT ${SPAN_COLS} FROM spans WHERE session_id = ? AND type = 'llm_turn' ORDER BY start_time ASC`).all(req.params.id) as Record<string, unknown>[];
    return rows.map(parseSpanRow);
  });

  // 每次工具调用明细
  app.get<{ Params: { id: string } }>('/api/session/:id/tools', async (req) => {
    const rows = db.prepare(`SELECT ${SPAN_COLS} FROM spans WHERE session_id = ? AND type = 'tool_call' ORDER BY start_time ASC`).all(req.params.id) as Record<string, unknown>[];
    return rows.map(parseSpanRow);
  });
  ...
  app.get<{ Params: { id: string } }>('/api/session/:id/diagnosis', async (req, reply) => {
    const session = db.prepare(`SELECT ${SESSION_COLS} FROM sessions WHERE id = ?`).get(req.params.id) as SessionSummary | undefined;
    if (!session) return reply.status(404).send({ error: 'session not found' });
    const rows = db.prepare(`SELECT ${SPAN_COLS} FROM spans WHERE session_id = ? ORDER BY start_time ASC`).all(req.params.id) as Record<string, unknown>[];
  ...
  app.get<{ Params: { id: string } }>('/api/session/:id/context', async (req) => {
    ...
    ).all(req.params.id) as { startTime: number; contextTokens: number; inputTokens: number; cacheCreationTokens: number; cacheReadTokens: number; model?: string }[];
    return rows.map((r) => ({ ...r, contextWindow: getModelContext(r.model) ?? null }));
  });

  app.get('/api/pricing', async () => {
    return db.prepare(`SELECT model, input_price as inputPrice, cache_creation_price as cacheCreationPrice,
      cache_read_price as cacheReadPrice, output_price as outputPrice, effective_from as effectiveFrom
      FROM pricing ORDER BY model`).all() as Pricing[];
  });

  app.put<{ Body: PricingBody }>('/api/pricing', async (req) => {
    const b = req.body;
    db.prepare(`INSERT INTO pricing (model, input_price, cache_creation_price, cache_read_price, output_price, effective_from)
      VALUES (?, ?, ?, ?, ?, ?)`).run(b.model, b.inputPrice, b.cacheCreationPrice, b.cacheReadPrice, b.outputPrice, b.effectiveFrom ?? null);
    return { ok: true };
  });

  app.get('/api/model-context', async () => {
    return db.prepare(`SELECT model, context_window as contextWindow FROM model_context ORDER BY model`).all();
  });

  app.put<{ Body: ModelContextBody }>('/api/model-context', async (req) => {
    const b = req.body;
    db.prepare(`INSERT INTO model_context (model, context_window) VALUES (?, ?)
      ON CONFLICT(model) DO UPDATE SET context_window = excluded.context_window`).run(b.model, b.contextWindow);
    return { ok: true };
  });
}
